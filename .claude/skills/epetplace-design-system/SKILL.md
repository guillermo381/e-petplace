...
| `Insignia` | Todo chip de estado/capa/distinción; `soloPunto` para celdas densas. **JAMÁS dispara una ACCIÓN** — un badge que filtra, navega, reintenta o cambia el estado es un `Boton` mal vestido. **PERO sí puede ABRIR SU PROPIA EXPLICACIÓN** (`onPress` en `distincion` desde S85-B24 y en `estado` desde S97-B): el usuario no cambia nada del sistema, se entera de qué es. *El criterio nunca fue la familia — es qué hace el toque.* Sin `onPress` sigue siendo `View` con `role="text"`; con él, el blanco táctil de 44 lo pone LA PIEZA. ⏪ *Esta fila decía «JAMÁS interactivo» y era falsa desde S85: la letra vieja se conserva acá porque el «jamás» sigue rigiendo — para las acciones, que es lo que siempre prohibió.* |
