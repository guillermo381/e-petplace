...
| `FichaMascotaHogar` | La ficha de una mascota en la Zona 1 del Hogar (S51-B2.2). Presentacional pura: AvatarMascota + nombre + UNA línea de estado en voz humana; la pantalla pasa el TEXTO ya traducido (riel i18n) y la `voz` decide la semántica — `alDia` punto verdeVital · `pideAtencion` punto ochre + warningText (pide sin gritar) · `conociendolo` neutral que invita. Tap → perfil (pressed resalta, jamás escala); sin badges ni CTA embebida. Memorial degrada: sin punto, voz neutra. Cero tokens nuevos |
