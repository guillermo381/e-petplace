...
export async function obtenerCitasGroomingDelDia(
  ...
  const { data, error } = await getClient()
    ...
    .select(
      'id, fecha, hora, estado, tipo_servicio, suscripcion_servicio_id, duracion_minutos, precio, empleado_id, mascota:mascotas(id, nombre, especie, foto_url, familia_id), tipo:tipos_servicio!inner(nombre, duracion_default_minutos), atencion:evento_atencion(estado, iniciada_en)',
    )
...
export async function obtenerCitaGroomingPorId(
  ...
  const { data, error } = await getClient()
    .from('evento_cita_servicio')
    .select(
      'id, fecha, hora, estado, tipo_servicio, suscripcion_servicio_id, duracion_minutos, precio, empleado_id, modalidad, direccion_snapshot, mascota:mascotas(id, nombre, especie, foto_url, familia_id), tipo:tipos_servicio!inner(nombre, duracion_default_minutos), atencion:evento_atencion(estado, iniciada_en)',
    )
    .eq('id', citaId)
    .eq('tipo.categoria', 'grooming')
    .in('estado', ['confirmada', 'en_curso', 'completada', 'no_show'])
