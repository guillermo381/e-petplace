...
**Estado: S44 CERRADA — 17 componentes, sistema exigible y YA CONSUMIDO por el flujo real del paseo (apps/prestador).**
...
- **17 componentes** (`src/components/`) — los 11 de S43, cada uno con su gate en dispositivo cerrado: Boton (5 variantes, loading 150ms sin layout shift, marca degrada en memorial) · Tarjeta (tintes, interactiva 0.99, sombra jamás animada) · Campo (nada se anima al tipear, slot de mensaje reservado) · Celda (jerarquía protegida, metadataMono fuerza la voz, pressed resalta jamás escala) · Separador · Insignia (jamás Pressable, dos familias, soloPunto con a11y forzada) · Encabezado (navegacion con centrado óptico / portada con la voz humana e isotipo gradiente default) · BarraTabs (pill accent.active por opacity, wiring expo-router en JSDoc, aria-selected directo) · Hoja (bottom sheet, gestos SM, back Android DOBLE VÍA — CERRADO, verificado en dispositivo por founder post-b0fae55) · Aviso (cola de-a-uno con ref síncrono) · EstadoVacio (dignidad, sin animación de entrada). **+ los 6 de S44**: CitaEnVivo (glow semántico dark / anillo 1.5+pill claro / memorial degrada; UNO por pantalla) · Esqueleto (+EsqueletoGrupo progressbar aria-busy; estático por ley) · AvatarMascota (foto→huella digna; `especie` reservada a D-288; nació el registro capaBg y violetText subió a #AE59FF) · Cronometro (mono display tabular-nums, por diferencia contra el server, pausadoEnMs congela) · EvidenciaFoto (.Capturar cámara directa/galería en Hoja/permiso con voz humana; .Thumbnail subiendo/subida/error — la foto jamás desaparece) · MapaRecorrido (nativo+web placeholder; EXIGE dev build — tiles de Google fuera de Expo Go desde SDK 53). Celda enmendada en S44: metadataMono + fin CONVIVEN apilados. Gate WCAG: 135 pares / 0 fallos.
- **Marca**: `src/brand/Isotipo.tsx` (path oficial; tinta/blanco/gradiente) + `assets/brand/` (8 logos + 3 isotipos).
- **Galería**: `src/gallery/TokenGallery.tsx` — tab "Tokens" en ambos apps; incluye la pantalla raíz completa del prestador (el template que S44 volvió pantalla real) + secciones de los 6 componentes de S44 con los 3 temas visibles (PanelTema).

## Gates

- WCAG: `pnpm verify:contrast` — 135 pares (S44: +Avatar +scrim), 0 fallos (regla de peor punto del gradiente con exención de cola verificada; tab inactivo exento por espec con medición informativa).
