/**
 * Isotipo e-PetPlace — el path oficial del Manual de Marca
 * (Iso_Estandar0.svg, viewBox 471.82×324), render vía react-native-svg
 * (funciona nativo + web).
 *
 * Variantes:
 *   'tinta'     → #1D1A2E (CTA/marca sobre claro — dosis prestador)
 *   'blanco'    → #FFFFFF (sobre oscuro o sobre gradiente)
 *   'gradiente' → los 6 stops EXACTOS del SVG de marca (vertical).
 *                 USO CERRADO: splash/logo (gradientLogo, ver palette.ts).
 */

import Svg, { Defs, LinearGradient, Path, Stop } from 'react-native-svg'

import { gradients, palette } from '../tokens/palette'

export const ISOTIPO_VB_W = 471.82
const VIEWBOX_W = ISOTIPO_VB_W
export const ISOTIPO_VB_H = 324
const VIEWBOX_H = ISOTIPO_VB_H
/** S99-B — SE EXPORTA en vez de copiarse. Lo pide `PinEnMapa`, que mete
 *  el isotipo adentro del cajón de la moto y **no puede anidar un `<Svg>`
 *  dentro de otro**. Exportar el path es lo único que evita el clon —y un
 *  clon de la MARCA es el peor de todos: divergiría sin que nadie lo note
 *  hasta verlos juntos. */
export const ISOTIPO_PATH = 'M213.06,302.96c-13.34,.28-26.62-.75-39.46-3.04-67.26-12-123.88-57.18-144.24-115.1-4.72-13.43-7.47-27.48-8.18-41.78-1.84-36.88,10.05-70.12,33.48-93.59,22.26-22.3,52.26-31.12,80.24-23.61,.12,.03,.23,.07,.35,.1-7.93,5.64-14.88,12.09-20.76,19.31-11.36,13.94-18.2,30.71-19.25,47.22-1.19,18.56,4.89,36.63,16.68,49.57,10.68,11.72,25.77,18.88,41.4,19.63,.89,.04,1.78,.06,2.67,.06,14.94,0,29.9-5.99,40.47-16.31,15.43-15.08,21.96-39.73,17.02-64.33h0c-4.19-20.88-15.65-39.78-32.33-54.01,9.17-2.74,19.04-4.7,29.53-5.8,1.15-.12,2.29-.22,3.42-.31,5.78-.45,10.1-5.5,9.65-11.28-.45-5.78-5.5-10.12-11.28-9.65-1.33,.1-2.67,.22-4,.36-17.94,1.88-34.42,5.98-49.09,12.2l-.12-.07-.16,.07c-6.02-2.96-12.3-5.32-18.74-7.05-35.33-9.49-72.92,1.37-100.55,29.06C12.14,62.33-1.92,101.21,.21,144.08c.81,16.32,3.96,32.36,9.35,47.7,22.83,64.95,85.77,115.51,160.36,128.82,12.69,2.26,25.75,3.41,38.89,3.41,1.56,0,3.12-.02,4.68-.05,5.8-.12,10.4-4.92,10.28-10.71s-4.96-10.38-10.71-10.28ZM158.39,36.29c18.08,11.62,30.53,29.19,34.5,48.93,3.55,17.7-.71,35.01-11.11,45.18-7.09,6.93-17.36,10.78-27.45,10.29-9.97-.48-20.02-5.26-26.88-12.79-7.86-8.63-12.06-21.37-11.25-34.09,.78-12.18,5.95-24.71,14.57-35.29,7.19-8.83,16.46-16.29,27.62-22.22Zm273.63-1.68C404.39,6.92,366.8-3.94,331.46,5.55c-6.44,1.73-12.72,4.1-18.74,7.05l-.16-.07-.12,.07c-14.67-6.21-31.15-10.31-49.07-12.19-1.35-.14-2.69-.26-4.02-.37-5.76-.47-10.83,3.87-11.28,9.65-.45,5.78,3.87,10.83,9.65,11.28,1.13,.09,2.27,.19,3.44,.31,10.48,1.1,20.34,3.06,29.51,5.8-16.68,14.22-28.14,33.13-32.33,54.01-4.93,24.6,1.59,49.25,17.03,64.33,10.56,10.32,25.52,16.31,40.46,16.31,.89,0,1.78-.02,2.66-.06,15.63-.75,30.72-7.91,41.4-19.63,11.79-12.94,17.87-31.01,16.68-49.57-1.05-16.52-7.89-33.29-19.24-47.22-5.88-7.23-12.83-13.68-20.76-19.32,.12-.03,.23-.07,.35-.1,27.98-7.51,57.98,1.31,80.24,23.61,23.43,23.47,35.32,56.71,33.48,93.59-.71,14.3-3.47,28.36-8.18,41.78-20.36,57.92-76.97,103.11-144.24,115.11-12.83,2.29-26.11,3.31-39.46,3.04-.07,0-.15,0-.22,0-5.7,0-10.38,4.56-10.49,10.28-.12,5.8,4.48,10.59,10.28,10.71,1.56,.03,3.12,.05,4.68,.05,13.14,0,26.21-1.14,38.89-3.41,74.59-13.31,137.53-63.87,160.36-128.82,5.39-15.33,8.53-31.38,9.35-47.7,2.14-42.88-11.93-81.75-39.59-109.48Zm-90.96,23.91c8.62,10.57,13.79,23.1,14.57,35.29,.81,12.72-3.39,25.46-11.25,34.09-6.86,7.53-16.91,12.31-26.88,12.79-10.09,.49-20.35-3.36-27.44-10.29-10.4-10.17-14.66-27.48-11.11-45.18,3.96-19.74,16.42-37.32,34.49-48.93,11.16,5.93,20.43,13.39,27.62,22.23Z'

/**
 * ⏪ S96-B — LOS SEIS STOPS SE DERIVAN DE `gradients.logo`, NO SE
 * REPITEN. Acá vivían los seis hex escritos a mano, y **`DIRECCION_ARTE`
 * §9bis.3 dice con todas las letras que su fuente única es
 * `packages/ui/src/tokens/palette.ts`** — o sea que esta copia
 * contradecía una letra firmada sin que nada lo dijera.
 *
 * **Verificado byte a byte antes de tocar:** los seis colores y los seis
 * `locations` de `gradients.logo` son idénticos a los que estaban acá, así
 * que el render no cambia — es deduplicación, no recalibración.
 *
 * **Cómo apareció, que es lo que vale:** no lo encontró una revisión del
 * código. Salió de MEDIR el corpus para la regla nueva del lint (D-781):
 * el censo de "hex en posición de valor de estilo" lo señaló junto con un
 * residuo de template del prestador. *Una duplicación de la rampa de
 * marca es exactamente la clase de dato que diverge en silencio: el día
 * que alguien afine un stop en `palette.ts`, el isotipo se queda con el
 * viejo y nadie se entera hasta verlo.* Es D-546 un piso más arriba.
 */
const STOPS: ReadonlyArray<{ offset: number; color: string }> = gradients.logo.colors.map(
  (color, i) => ({ offset: gradients.logo.locations[i], color }),
)

export type IsotipoVariant = 'tinta' | 'blanco' | 'gradiente'

export function Isotipo({
  size = 32,
  variant = 'tinta',
  color,
}: {
  /** Alto en px; el ancho se deriva del aspect ratio del viewBox. */
  size?: number
  variant?: IsotipoVariant
  /** S61-B8 (enmienda ADITIVA, letra founder de la bienvenida del
   *  prestador: isotipo en tealDark): override puntual del color de
   *  relleno — solo aplica sobre tinta/blanco, jamás sobre gradiente.
   *  S81 (respuesta al pedido de C, bienvenida-dia1): esto ES el fill
   *  custom — el tono-sobre-tono (p.ej. marca de agua teal) se hace con
   *  esta prop; existía desde S61 y el pedido se contesta con el
   *  literal, cero código nuevo. El gradiente sigue INTOCABLE. */
  color?: string
}) {
  const width = (size * VIEWBOX_W) / VIEWBOX_H
  // ⏪ S96-B — los dos literales también salen de `palette`. Y acá había
  // una TRAMPA DE NOMBRE que casi me cuesta un cambio de color: el JSDoc
  // de arriba llama «tinta» a `#1D1A2E`, pero **`palette.tinta` es
  // `#221E19`** — otro hex. El token correcto es `textLight0`, que se
  // llama distinto y ES el valor que este archivo venía usando desde S43.
  // *Sustituir por el token que comparte el NOMBRE habría cambiado la
  // marca sin que ningún gate lo viera: el typecheck pasa igual y WCAG no
  // mide el isotipo.* Se verificó el hex, no el nombre.
  const fill =
    variant === 'gradiente'
      ? 'url(#isoGrad)'
      : (color ?? (variant === 'tinta' ? palette.textLight0 : palette.white))

  return (
    <Svg width={width} height={size} viewBox={`0 0 ${VIEWBOX_W} ${VIEWBOX_H}`}>
      {variant === 'gradiente' ? (
        <Defs>
          <LinearGradient id="isoGrad" x1="0.5" y1="0" x2="0.5" y2="1.456">
            {STOPS.map((s) => (
              <Stop key={s.offset} offset={s.offset} stopColor={s.color} />
            ))}
          </LinearGradient>
        </Defs>
      ) : null}
      <Path d={ISOTIPO_PATH} fill={fill} />
    </Svg>
  )
}
