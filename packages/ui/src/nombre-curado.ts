/**
 * nombreCurado — LA CAJA DEL NOMBRE DE PRODUCTO, una sola para las dos
 * caras (subida a `packages/ui` en S100-B).
 *
 * ═══════════════════════════════════════════════════════════════════
 * 🔴 POR QUÉ SUBE, y no es prolijidad: **si vive en `apps/cliente`, el
 * espejo del vendedor no puede consumirla sin duplicarla** — y una
 * segunda copia es el mismo defecto con otro disfraz.
 *
 * **El caso, medido por C sobre el catálogo real:** 42 % de los 563
 * nombres viene EN MAYÚSCULAS (`CANADA LITTER`). Si solo una de las dos
 * caras cura la caja, **el vendedor ve `CANADA LITTER` y la familia ve
 * `Canada Litter` del mismo producto** — dos superficies pintando el
 * mismo dato y desacordando **sin síntoma**, que es H-001 otra vez.
 *
 * ⇒ **N17 aplicada a la LÓGICA y no solo a la forma:** la pieza que las
 * dos caras montan (`TarjetaProducto`) y la función que prepara su dato
 * viajan juntas. *Mientras la función vivía en una app, el problema no
 * se podía arreglar bien desde ninguna de las dos puntas.*
 *
 * ⚠️ **Sube TAL CUAL, sin tocar una línea de su lógica** — con su
 * medición hecha por C: **20/26 nombres cambiados · 26/26 idempotente ·
 * los 6 que ya traían caja propia, intactos.** *Una función que se muda
 * y de paso se «mejora» llega sin su medición, y entonces nadie sabe si
 * el que falla es el traslado o la mejora.*
 *
 * ── LO QUE HACE Y LO QUE NO ────────────────────────────────────────
 * **Cambia la CAJA. No inventa una palabra.** Nada de agregarle
 * «Arena aglomerante» a un nombre que no lo dice: *un descriptor
 * deducido del nombre es verosímil y falso* (L-139).
 *
 * ── ⚠️ EL ORDEN DE SUS REGLAS ES LA REGLA (hallazgo de C) ──────────
 * Su regla de *siglas de ≤2 letras se conservan* —escrita para salvar
 * `ID`/`KD` de las dietas veterinarias— **se adelantaba a la de enlaces
 * y devolvía «Control DE Peso», «Medianas Y Grandes»**: las tres
 * palabras más frecuentes del catálogo, gritando. **Peor que no curar,
 * porque parece descuido.** *Leyendo el código las dos reglas parecían
 * independientes; solo corriéndola contra nombres vivos se vio que una
 * pisaba a la otra.*
 */
