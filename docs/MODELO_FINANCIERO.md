...
> **ENMIENDA S79 (migración `20260727190000` — consecuencia mecánica de dos decisiones founder: el voto (a) del invariante "activo ⇒ ofertable" + LETRA_ALTA_S79 §4 "bancarios son requisito de COBRO, no de activación"):** el CHECK pasa a **VACÍO-O-COMPLETO** — `'{}'` es legal en cualquier estado (la cuenta activada por invitación todavía no dio bancarios), y un `datos_bancarios` MALFORMADO sigue rebotando (la validación estructural de esta Decisión queda entera; probado en fixture). La premisa original del CHECK (el wizard capturaba bancarios ANTES de la activación admin) murió con el alta por invitación. **La exigencia de COMPLETITUD se muda a donde pertenece: el arco de pagos** — la liquidación real (Kushki) exige bancarios completos como gate propio; hasta ese arco, una cuenta activa sin bancarios puede operar cobros SIMULADOS y no puede liquidar (declarado, no accidental). `activar_prestador` activa la cuenta `pendiente_validacion` en la misma transacción; `suspendida`/`cerrada` JAMÁS se reactivan por esa vía (rebote `cuenta_no_activable` — reactivar es §7.7).
...
> **ENMIENDA S79 (founder — LA REFORMA DEL PRECIO; letra completa en `MODELO_PASEO` §6.2, migración `20260727230000`):** la BASE del total del período deja de ser `precio-por-salida × N` y pasa a ser **`precio_mensual_plan` FIJO** — el plan es suscripción mensual: la familia paga el mes, use 5 salidas o use 20. Todo lo demás de esta Decisión queda INTACTO: un pago por período, variante (b) con N devengos al cierre, y el **unitario efectivo pasa a ser DERIVADO** (`mensual ÷ N generadas`) — sigue siendo la base del devengo, y queda DECLARADO que **NO es estable entre períodos** (N varía con el mes; ningún reporte puede asumir unitario fijo). Sin `precio_mensual_plan` declarado NO hay plan que contratar ni renovar (`plan_no_ofrecido` — jamás un fallback al precio suelto). `precio_plan` (por-salida) jubilada. El cobro sigue simulado; el ciclo real es del arco de pagos.
