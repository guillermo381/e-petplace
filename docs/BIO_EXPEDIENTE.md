...
> **Nota S67 (fundación V0):** la PROCEDENCIA nació en schema en la
> fundación V0 (S67) — `eventos_mascota.procedencia`
> ('declarado_por_familia' | 'verificado_por_prestador', exigida por
...
- ✅ **RPC nueva S46**: `registrar_vacunas_de_carnet(p_mascota_id, p_vacunas jsonb)` — **SECURITY INVOKER** (la RLS del dueño ES la puerta: rama titular de `vacuna_insert` por `mascotas.user_id`; hereda A SABIENDAS la brecha co-dueño D-294), atómica (una fila mala = cero escritas), errores tipados por shape (`sin_acceso_mascota` / `vacunas_vacias` / `item_invalido: <n>: <motivo>`); el trigger `_trg_vacuna_crear_evento` crea los eventos padre. Su insumo es la Edge Function `extract-vacuna` **v16**, que desde S46 vive en el monorepo re-targeteada al modelo real (cierra la pata extract-vacuna de D-283; el resto de D-283 — tablas muertas v2 — sigue abierto). **Enmienda S47 (migración `20260709010000`):** la firma pasa a `(p_mascota_id, p_vacunas, p_archivo_url text default null)` — el carnet SE CONSERVA como evidencia (decisión de producto D-308): path del bucket **privado** `mascotas` en la carpeta del dueño (guard tipado `archivo_invalido`), el MISMO archivo respalda las N filas del lote en `evento_vacuna_aplicada.archivo_url`; la lectura se sirve con signed URLs. **Enmienda S48 (cierre del carnet, gate con carnet físico real):** (a) la vacuna se guarda con **nombre+fecha; `tipo_vacuna` es opcional** (null honesto, editable — decisión founder: los carnets reales no lo rotulan); la extracción lo INFIERE desde el nombre comercial contra un vocabulario cerrado (proto-catálogo D-008, dos dialectos anotados ahí). (b) `extract-vacuna` corre **Sonnet** con reglas de atribución sticker↔columna FECHA (version 24 ACTIVE = prompt v21; el gate destapó "verosímil pero falso" — fechas impresas de sticker como fecha de aplicación, L-139: techo de modelo, no de prompt). (c) La vacuna es **evento de fecha-sola**: `fecha_evento` ancla la medianoche UTC del día solo para ordenar y el timeline muestra el día calendario SIN hora (`fecha_sola` en la puerta única; D-312 lleva la precisión al modelo cuando nazca el segundo tipo). (d) Hallazgo operativo: `_trg_eventos_update_ultimo` es solo-INSERT — todo borrado de eventos debe recomputar `ultimo_evento_*` del perfil vigente (nota en D-307).
