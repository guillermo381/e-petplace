...
> **Versión:** v1.30 (con enmiendas S14 + S15 + S16 + S19 + S21 + S42 + S48 + S54 + S59 + S68 + S71 + S79 + S80 + S81 + S82 + S83 + S84 + S86 + S88)
> **Última actualización:** 4 Ago 2026 — Sesión 86: **REGLA 86 — `merge --no-ff` SIEMPRE entre pistas; rebase/squash/cherry-pick PROHIBIDOS, y el ANCLA de un OTA es el commit de FUSIÓN, jamás el último de la rama** (un hash que un acta cita tiene que seguir resolviendo; evidencia: 11 merges y 4 publishes de S86). **+ SEGUNDA ENMIENDA AL ESLABÓN ③ — EL INSTRUMENTO: gana `eas update:view`; `update:list` queda DECLARADO al lado con su fecha, no borrado.** *Una orden que manda un instrumento ciego al dato que ella misma exige se corrige por el instrumento* — `update:list` no muestra el `gitCommitHash`, que es exactamente lo que el eslabón verifica. Resuelve la divergencia con `METODO_TRES_PISTAS` §2 deber ③ (que ya lo mandaba) declarada horas antes en esta misma sesión. **+ ENMIENDA A LA REGLA 84, ESLABÓN ③ — UN GROUP PUBLICADO YA NO ALCANZA: un publish sin el verde de `scripts/verify-ota.mjs` NO se declara cerrado.** *El eslabón no cambia de contenido: cambia su instrumento.* `update:list` describe **lo que guardaste**; el guard mide **lo que se sirve** — y solo lo segundo llega al teléfono (evidencia: el 4-ago el OTA del prestador salió con los tres eslabones verdes y el founder no vio nada). Comando y costo aceptado en `METODO_TRES_PISTAS` §3bis; su hermana es **D-649** (no hay forma de forzar la búsqueda de update desde la app). **+ DIVERGENCIA DECLARADA y NO resuelta:** este eslabón manda `update:list` y `METODO` §2 deber ③ lo prohíbe a favor de `update:view` — a la mesa. Sesión 83: **ENMIENDA A LA REGLA 84 — SON CUATRO ESTADOS, y el que faltaba es el PRIMERO: «en mi rama». El estado de una rama respecto de SÍ MISMA no dice nada sobre si su trabajo llegó** (se verifica con `merge-base --is-ancestor` contra `origin/main`, jamás con una lista propia; evidencia: 11 commits declarados pendientes estando los 11 en main — D-607). + **ENMIENDA A LA REGLA 82 — nace el PASO ⓪ de la veda: quien publica pide la congelación NOMBRANDO a quiénes espera y no bundlea sin confirmación AL MOMENTO** (una congelación de hace tres turnos no es una congelación; evidencia: el incidente C17). + **ENMIENDA A LA REGLA 80 — muere la lámina HTML como instrumento de diseño de PANTALLA; el ciclo es `UI real sin cablear → gate en dispositivo → cableado`** (la lámina sobrevive solo para comparar variantes de un token barato). Sesión 82. Reglas **84** (los CUATRO eslabones del cierre: pathspec obligatorio · `cat-file`+`merge-base` por CONTENIDO · group publicado · y lo que consumís sigue vigente) y **85** (worktree por pista = la PRIMERA decisión de una sesión paralela, pendiente). **El arranque de la sesión del prestador vive en `docs/relevamientos/2026-07-31-s82-acta-del-metodo.md` §5** y el censo de enmiendas de ley de S82 en `…-s82-censo-de-enmiendas.md` — se REFERENCIAN, no se copian.
...
- **v1.30 (5 Ago 2026 — S88):** **regla 87 · aparato conectado — UN APARATO CONECTADO NO ES UN APARATO LIBRE.** *(Propuesta por la pista D, firmada por la mesa. Es la forma de la regla 85 —el worktree por pista— aplicada al HARDWARE: el aislamiento del árbol no sirve de nada si las cuatro pistas comparten un teléfono.)*
  ...
  **LOS DOS INCIDENTES QUE LA FUNDAN, y son de dos pistas distintas — que es lo que la vuelve regla y no anécdota:**

  | incidente | qué pasó |
  |---|---|
  | **los taps de C** | cayeron en el campo *Description* de una pantalla que no era la suya |
  | **los intents de D sobre `emulator-5554`** | **robaron el foco DOS VECES** sobre una pantalla del prestador «con `s87recep` y el teclado arriba» — y **cambiaron dos contraseñas reales** |

  **EL COSTO MEDIDO, que es lo que hace la regla exigible:** las credenciales de `+s87recep` y `demo-prestador` **dejaron de autenticar**, y la pista C perdió tiempo midiendo si había roto algo. *El audit log muestra las dos escrituras y por CAMINOS DISTINTOS —`demo-prestador` por re-autenticación (login inmediatamente antes, sin `recovery_requested`); `+s87recep` por el reset completo (`recovery_requested → login → user_updated_password`)—* **lo que confirma que fueron dos pantallas distintas, no un mismo error repetido.**

  > **Y la lectura que trasciende el caso:** *un aparato compartido convierte el trabajo de una pista en un ACTO REAL sobre el estado de otra* — y esos actos **no tienen conflicto de merge que los frene**. Un `git` mal usado da un choque ruidoso; un intent mal dirigido **funciona perfectamente** y deja el daño escrito en la base de datos.

- **v1.29 (4 Ago 2026 — S86):** **regla 86** — el trabajo de una pista entra con **`merge --no-ff`**; **rebase, squash y cherry-pick quedan PROHIBIDOS entre pistas** *(reescriben o duplican hashes, y este canon cita hashes en cada acta, deuda y ficha de OTA: `cat-file` y `merge-base` tienen que seguir contestando meses después)*. **Segunda mitad: el ANCLA de un OTA es el commit de FUSIÓN, no el último de la pista** — el bundle se arma desde `main`, así que reportar el hash de la rama es reportar un árbol que nunca se bundleó. Evidencia: 11 merges y 4 publishes en S86, todos verificados por contenido.
- **v1.28 (4 Ago 2026 — S86):** **segunda enmienda al eslabón ③ el mismo día — EL INSTRUMENTO.** Gana **`eas update:view`**; **`update:list` queda DECLARADO al lado con su fecha, jamás borrado** *(una versión borrada no se puede auditar: quien encuentre un reporte viejo citándola tiene que poder ver que citaba la letra vigente de su día)*. **El criterio, que vale más que el caso: una orden que manda un instrumento ciego al dato que ella misma exige se corrige POR EL INSTRUMENTO** — no se ablanda la exigencia para que el instrumento alcance. `update:list` no muestra el `gitCommitHash`, que es exactamente lo que el eslabón verifica. **Resuelve la divergencia con `METODO_TRES_PISTAS` §2 deber ③** —que ya mandaba `update:view` con esta misma razón medida— **declarada en v1.27 horas antes**: dos letras firmadas que se contradicen son peores que una equivocada. Enmendados los DOS documentos en el mismo commit (L-198).
- **v1.27 (4 Ago 2026 — S86):** enmienda al **eslabón ③ de la regla 84** — *"cerrado"* deja de bastar con el group publicado: **un publish sin el verde de `scripts/verify-ota.mjs` no se declara cerrado.** El eslabón no cambia de contenido, **cambia su instrumento**: `update:list` describe lo que GUARDASTE, el guard mide lo que SE SIRVE. Origen: el 4-ago el OTA del prestador salió con los tres eslabones verdes y el founder no vio nada — ninguna verificación preguntaba qué le responde el servidor a un aparato. Comando, tres casos y **costo aceptado** (L-197: si no puede medir sale ROJO ⇒ una caída de EAS frena un cierre; escape ruidoso `--sin-builds`) en `METODO_TRES_PISTAS` §3bis. Hermana: **D-649**. **Divergencia declarada y NO resuelta:** este eslabón manda `update:list`, `METODO` §2 deber ③ lo prohíbe a favor de `update:view`.
...
1. **`git commit -- <rutas>` (forma PATHSPEC), SIEMPRE. `git commit` a secas queda PROHIBIDO en paralelo.** El índice (`.git/index`) es UNO por repo: `git add` acota lo que VOS agregás, `git commit` se lleva el índice ENTERO, y con tres pistas esa ventana se llena (D-586: tres arrastres, dos atribuciones perdidas). La forma pathspec **ignora el índice** — es la única exacta que git ofrece hoy. **TRES trampas medidas:** el `-m`/`-F` va **ANTES** del `--` (el `--` consume todo lo que sigue) · la forma pathspec **no ve archivos sin trackear** (`git add -N` o `git add -- <ruta>` primero, acotado) · **y la tercera, que faltaba:**

   > ### 🔴 **LA FORMA PATHSPEC TAMPOCO VE LOS BORRADOS QUE VIVEN EN EL ÍNDICE.**
   >
   > **Se DEDUCE del mecanismo que esta misma regla explica** —*«la forma pathspec ignora el índice»*— **pero no estaba entre sus consecuencias listadas, y quien leía las dos primeras creía estar cubierto.** *Una regla que enumera sus trampas enseña que la lista es completa.*
   >
   > **El caso, medido por B (S107, 29-ago-2026):** un `git mv` deja el borrado del archivo viejo **en el índice**. El commit por pathspec se lo come **sin avisar** ⇒ **el archivo viejo llega a `main` junto al nuevo, sin export, como código muerto que nada señala.** *Ni el typecheck ni el lint lo ven: un archivo huérfano compila perfecto.* **Lo destapó un merge fallido, no una revisión** — que es lo que lo vuelve caro: entre el commit y el síntoma puede pasar una sesión entera.
   >
   > **Lo operativo:** todo renombre o borrado **nombra explícitamente el archivo viejo en el pathspec** (`git commit -m "…" -- <nuevo> <viejo>`), y **se verifica por AUSENCIA** —`ls` o `git cat-file -e HEAD:<viejo>` **tiene que fallar**— antes de declararlo cerrado. *La verificación por contenido del eslabón ② prueba que lo nuevo llegó; sólo una verificación por ausencia prueba que lo viejo se fue.*
...
3. **"Cerrado" recién cuando está en un GROUP PUBLICADO**, verificado con **`eas update:view`** desde `apps/<app>/` (L-166: se lee al momento, jamás de memoria ni del último publish propio).

   > **☠️ LO QUE ESTA LÍNEA DECÍA HASTA EL 4-AGO-2026, y no se borra: `eas update:list`.** *Queda declarado al lado con su fecha porque **una versión borrada no se puede auditar**: quien encuentre un reporte viejo citando `update:list` tiene que poder saber que citaba la letra vigente de su día, y no que se equivocó.*
   >
   > **ENMIENDA S86 — FIRMADA POR LA MESA: GANA `update:view`.** El porqué viaja con la letra y es el que la decide: **`update:list` NO MUESTRA EL `gitCommitHash`, que es exactamente el dato que este eslabón verifica.**
   >
   > > ### **UNA ORDEN QUE MANDA UN INSTRUMENTO CIEGO AL DATO QUE ELLA MISMA EXIGE SE CORRIGE POR EL INSTRUMENTO.**
   >
   > *No se ablanda la exigencia para que el instrumento alcance: se cambia el instrumento.* **`METODO_TRES_PISTAS` §2 deber ③ ya mandaba `update:view` con esta misma razón medida** — la divergencia entre los dos documentos se declaró al enganchar el guard (S86) y **se resuelve acá, en el mismo commit que la nombra, y a favor del que podía ver el dato.**

   > **➕ ENMIENDA S86 — FIRMADA POR LA MESA: UN GROUP PUBLICADO YA NO ALCANZA. UN PUBLISH SIN EL VERDE DE `scripts/verify-ota.mjs` NO SE DECLARA CERRADO.**
   >
   > **Por qué este eslabón, y no uno nuevo:** el defecto no era que faltara un estado — era que **este estado se estaba midiendo con el instrumento equivocado.** *`update:list` describe **lo que guardaste**; el guard mide **lo que se sirve**.* **Son dos cosas distintas y solo la segunda llega al teléfono.**
   >
   > **LA EVIDENCIA (4-ago-2026):** el OTA del prestador salió con los tres eslabones verdes —12 hashes ancestros de `origin/main`, árbol limpio, group propio como cabeza del branch— **y el founder no vio nada.** Ninguna verificación de esta regla podía fallar, **porque ninguna preguntaba qué le responde el servidor a un aparato que pregunta como aparato.**
   >
   > **Comando, tres casos y costo aceptado: `docs/METODO_TRES_PISTAS.md` §3bis** *(fuente única — se referencia, no se copia)*. **El costo, resumido acá porque cambia lo que esta regla obliga:** por **L-197** el guard sale **ROJO cuando no puede medir** ⇒ **una caída de EAS frena un cierre.** Se acepta a propósito; el escape es `--sin-builds`, explícito y ruidoso, y quien lo use lo declara en el mismo mensaje del ancla.
   >
...
>
> *`rebase` y `squash` **reescriben** los hashes; `cherry-pick` **duplica** el contenido bajo un hash nuevo y deja el original huérfano.* **Las tres dejan a las actas apuntando a nada** — y el canon de esta casa **cita hashes en cada acta, en cada deuda y en cada ficha de OTA**. `--no-ff` conserva **cada commit de la pista verbatim**, así que `cat-file` y `merge-base` siguen contestando meses después.

**Y la segunda mitad, que es de dónde salió la regla: el ANCLA del OTA es el commit de FUSIÓN, no el último de la pista.** *El bundle se arma desde `main`: lo que viaja es el árbol fusionado, no el de la rama.* **Reportar el hash de la pista como ancla es reportar un árbol que nunca se bundleó** — y es indistinguible de un reporte correcto hasta que alguien intenta reproducirlo.

**Evidencia (S86):** **11 merges `--no-ff`** sobre `main` y **cuatro publishes**, todos anclados en su commit de fusión y verificados por `cat-file`+`merge-base` contra `origin/main` (eslabón ② de la regla 84). *El precedente en contra es de S82: una pista reportó publicado algo que vivía en un commit huérfano.*

**Regla 85 — WORKTREE POR PISTA ES LA PRIMERA DECISIÓN DE UNA SESIÓN PARALELA (S82 — DECISIÓN DE ARRANQUE, no un hecho).** La regla 76(f2)/84 mitiga el índice compartido pero **no cierra la clase**: en git los archivos no tienen dueño, y ningún hook puede distinguir el arrastre. **La cura de la clase entera es un árbol e índice por pista** (la candidata `worktree-detached` de la regla 82). **Se decide al ARRANCAR la sesión del prestador, con el árbol limpio** — decidirlo a mitad de sesión es una migración, no una decisión. **➕ FIRMADA POR LA MESA (S85) — deja de ser pendiente.** *No es letra nueva: es su estatuto cambiando, y lo que lo justifica es la evidencia de un solo día.* **Las TRES veces que el árbol se movió entre la declaración y el bundle:** ① `Boton.tsx` sin commitear en el primer publish (B) · ② `TokenGallery.tsx` **+90/−73 tres minutos después de confirmar** (B) · ③ **un commit de A posterior a las dos confirmaciones** (A). **Las tres son la misma causa y ninguna es un descuido: son tres pistas escribiendo en UN árbol y UN índice** — con worktree por pista, **ninguna ocurre**, porque el árbol de quien bundlea no puede ensuciarse con trabajo ajeno. **Y lo que la vuelve estructural y no disciplinaria: la tercera fue A**, que tenía la disciplina escrita, la había exigido a las otras dos ese mismo día, y aun así la rompió. *Una regla que su propio autor incumple con el instrumento en la mano no falla por falta de rigor: falla porque el entorno la hace fácil de romper.* **Paga también D-586** (el índice compartido — dos `index.lock` en la misma sesión). **Lo que NO resuelve, declarado: el reparto. Worktree separa árboles, no territorios.** Queda escrito acá como lo que es: **pendiente de la primera decisión de esa sesión, no aplicado.** *(Lo que cuesta si no se toma, medido en S82: tres arrastres, dos atribuciones perdidas, casi un OTA con ancla que no contenía su código, dos bloqueos de `index.lock`, un `checkout` fallado en silencio, y el efecto lateral del hook — el WIP roto de una pista frena el commit de otra.)*
