...
> **⚠️ LETRA_EQUIPO v3 — ENMIENDAS S76 (al FIRMARSE `docs/LETRA_RECEPCION_S76.md`,
> founder 24 Jul 2026 — piso de literal S76-A0/A0bis).**
> **(1) EL SELECTOR PASA A DOS TOGGLES:** Administrador + Prestador,
> independientes; **recepción es la AUSENCIA de los dos** (no una opción), y
> `profesional` pasa a **derivado — tiene ≥1 chip**. Enmienda `LETRA_ROLES_EQUIPO`
> §1-§2 (*"la puerta no pregunta lo que ya sabe"*).
> **(2) LA LEY MADRE NUEVA — LA VENTANILLA GATEA POR MEMBRESÍA, LO CLÍNICO POR
> CHIP:** recibir el walk-in · registrar el cobro presencial · registrar la
> llegada = **cualquiera activo**; escribir el expediente · firmar la vacuna ·
> abrir caso · la nota = **solo el chip**. No es motor nuevo — es la fotografía de
> tres decisiones ya vivas (cobro presencial S69, mostrador S72, gate clínico
> D-490), con su literal en A0 (puntos 5, 10, 4).
> **(3) HECHOS DE PUERTA vs DECISIONES DE AGENDA:** recepción escribe HECHOS
> (llegó · walk-in · cobro), **jamás DECISIONES** (fecha, profesional, servicio).
> **(4) D-522 CORREGIDA — la enmienda S75(3) de arriba estaba mal leída:** la
> agenda NO está "abierta al negocio entero", está **CERRADA DE MÁS** — el
> predicado `cita_*` restringe al `empleado_id` PROPIO (A0 punto 3), el empleado
> ve solo lo suyo. *"Recepción ve todo"* es **construcción pura** y son DOS
> permisos (la cita con su `empleado_id`, hoy sin lector — A0 punto 8 —, y
> resolver ese id a un nombre, hoy bloqueado por `empleados_self` — A0 punto 7).
> Bug vivo: `registrar_atencion_mostrador` DEFINER en N>1 deja `empleado_id=NULL`
> → la recepcionista crea un walk-in que después no puede leer.
> **(5) EL GATE CLÍNICO PASA A LEER EL CHIP por `es_medico`** (no `profesional`
> genérico) — cierra el agujero del groomer con toggle Prestador que obtendría
> lectura/firma clínica. Ventana gratis hoy (0 filas `profesional` — A0 punto 6).
> **(6) "LLEGÓ" = timestamp `llegada_en`, jamás estado** (el check-in no existe
> hoy en ninguna capa — A0 punto 9); **D-524 se resuelve SIN estado nuevo**
> ("pendiente de firma" es derivable: walk-in médico sin su evento de vacuna).
> **La CASA DE RECEPCIÓN (cuatro zonas: la puerta · el día · registrar atención ·
> adelante) es BOCETO S76, construcción S77.** Fuente: `LETRA_RECEPCION_S76`.
> **Los dos frenos de la tanda, RESUELTOS (ADDENDUM 3, mismo día):** la reagenda
> clínica es **P22** en `POLITICAS` v1.10 (custodia conserva P20 — la reserva de
> S70; la mesa había dictado "P20" leyendo un resumen en vez de la fuente:
> L-166 contra la mesa) · el literal es+en del aviso de administrador está
> depositado en `LETRA_ROLES_EQUIPO` §6 (firmado S76; queda como CRITERIO DE
> ACEPTACIÓN del motor administrativo — enumera poderes que hoy no existen,
> D-513). El motor administrativo sigue titular-only (D-513) — la enmienda (2)
> del bloque S75 rige: el toggle Administrador **no se ofrece hasta que su motor
> exista** (arco v2).
...
### §14bis — LETRA_EQUIPO **v2**: LOS ROLES (S74, disparada por la FIRMA)

> **v2 nace al FIRMARSE `docs/LETRA_ROLES_EQUIPO_S74.md`** (founder, 22 Jul
> 2026) — la propia letra declaraba esta enmienda como su efecto. La v1 de
> abajo NO se borra: v2 la **completa** (v1 = dónde viven los roles en el
> motor · v2 = **qué significan** y **quién los entrega**).

**LOS TRES ROLES, con la ley madre:** `recepcion` es **EL PISO** (se nombra y
**no recibe chips**: cualquiera que entró y no tiene servicios ES recepción) ·
`profesional` suma **chips de oficio ACUMULABLES** (los servicios que presta) ·
`administrador` suma **plata · gestión · equipo**, y sus chips son
**OPCIONALES**. **EL ACCESO CLÍNICO VIENE DEL CHIP, JAMÁS DEL CARGO** — así
el dueño-que-atiende no elige entre ser negocio y ser vet, y el gerente que
no atiende **no lee una sola historia clínica** (si el cargo diera acceso,
sería el agujero de D-464 reabierto por la puerta de la administración).

**Vocabulario en piedra:** `especialista` **PROHIBIDA** como nivel de acceso
(`MODELO_PRESENCIA` §5-§6: la plataforma jamás estira una credencial;
una-palabra-dos-significados es el error del `'otro'` de S72) ·
`trabajador` fuera (describe relación laboral, no oficio) · queda
**`profesional`**.

**EL TITULAR (§4 de la letra):** es **quien crea la cuenta**, y su alta **no
nace de un formulario** — la cura inicial es humana (equipo de e-PetPlace).
Por eso **no se quita desde la app ni aparece en el selector**, y el candado
técnico "un negocio no puede quedar sin titular" no hace falta en v1.
**Solo el TITULAR nombra administradores** (firma founder S74 — la mesa
...
PRESIDE** con su acción al lado · **el flujo de dos pasos NO se tira**
(invitar sin saber todavía qué va a hacer es legítimo) · **invitar un mail
que ya pertenece a la cuenta LO DICE** (jamás invitación fantasma — hallazgo
de campo) · **quitar un rol NO borra lo escrito** (el expediente es
append-only: el acceso se pierde de ahí en adelante).

**EL AVISO ANTES DE CONFIRMAR ADMINISTRADOR (§6):** consentimiento
informado — la superficie **enumera qué se entrega** (plata, gestión,
equipo, y los chips si los tiene) **ANTES** de confirmar. **Nota de oficio
firmada: el botón dice "dar el rol", jamás "hacerla administradora" — el
género no se resuelve desde un nombre.** *(El LITERAL del string está
firmado pero no viajó al depósito — hueco declarado, L-142.)*

**LA INVITACIÓN — las dos vías y sus candados (§7):** por **correo** (cuando
exista el productor: hoy **NO existe**, D-508) y por **LINK compartible**
(copiar + share sheet nativo del SO, WhatsApp incluido — **el share sheet NO
requiere Meta/WABA**). **Los cuatro candados del link, en piedra:**
**vence a las 48 HORAS** (firma founder S74, bajó de 72) · **un solo uso** ·
**CANCELABLE** — hoy no existe cancelación, y con links pasa de incomodidad
a **bloqueante** · **el rol ADMINISTRADOR JAMÁS viaja en un link** (un link
se reenvía; **el poder se da mirando**). **Auditoría:** quién invitó, cuándo,
con qué rol **y por qué vía** (D-509).

**LO QUE ESTE MODELO JAMÁS HACE:** acceso clínico por cargo administrativo ·
roles que se auto-asignan desde una pantalla · links eternos, reutilizables o
irrevocables · el rol de administrador entregado sin que el que lo entrega
lea qué entrega · borrar lo que un profesional escribió al quitarle el rol ·
una superficie que afirma haber enviado algo que no se envió.

**ESTADO DE CONSTRUCCIÓN AL FIRMARSE (honesto):** el motor de roles existe
(S73) y la ventana de equipo está construida (S74-B) — **pero el arco está
DESCONECTADO: la app del prestador resuelve "qué negocio opero" por
titularidad, así que la degradación por rol es hoy INALCANZABLE incluso para
el founder** (**D-512**, con su regla de orden: **LA PUERTA VA ÚLTIMA**). La
letra queda firmada; su superficie espera el switch.
...
- **v1.5 (24 Jul 2026 — S76, disparada por la firma de `LETRA_RECEPCION_S76`):** la sección 14 pasa a **LETRA_EQUIPO v3** con el bloque de enmiendas S76 — el **selector pasa a DOS TOGGLES** (recepción = ausencia de ambos; `profesional` derivado de ≥1 chip), la **ley madre nueva** (la ventanilla gatea por MEMBRESÍA, lo clínico por CHIP), el principio **hechos de puerta vs decisiones de agenda**, la **corrección de D-522** (la agenda está cerrada de más, no abierta — la enmienda S75(3) estaba mal leída), el **gate clínico por `es_medico`** (chip, no cargo), **"llegó" como timestamp** (D-524 sin estado nuevo), y la casa de recepción como boceto S76/construcción S77. Piso de literal: relevamientos S76-A0/A0bis. **DOS FRENOS declarados y RESUELTOS el mismo día (ADDENDUM 3):** la reagenda clínica tomó **P22** (`POLITICAS` v1.10; custodia conserva P20 — la mesa había dictado "P20" leyendo un resumen, L-166 contra la mesa) · el literal del aviso de administrador llegó por mano del founder y vive en `LETRA_ROLES_EQUIPO` §6 (criterio de aceptación del motor administrativo). El motor administrativo sigue titular-only (D-513); el toggle Administrador no se ofrece hasta su motor (arco v2). Las versiones previas del §14 se conservan íntegras.
