...
> **DOS FRENOS abiertos:** el número de la reagenda clínica de §8 COLISIONA (la
> letra dice "P20" pero P20 está reservada a la custodia D-405 — el libre es P22;
> `POLITICAS` NO se toca hasta que el founder resuelva) · el literal es+en del
> aviso de administrador (§6 de `LETRA_ROLES_EQUIPO`) no llegó (L-142). El motor
> administrativo sigue titular-only (D-513) — la enmienda (2) del bloque S75 rige:
> el toggle Administrador **no se ofrece hasta que su motor exista** (arco v2).
