-- ═══════════════════════════════════════════════════════════════════════════
-- REVERSA de 20260807010000_s89a_matricula_de_la_persona.sql
-- Escrita ANTES. Restaura los tres productores sin gate; después borra
-- helpers y columnas.
-- ⚠️ REVERTIR REABRE EL HUECO: una cita médica vuelve a poder asignarse a un
-- profesional sin matrícula, y el papel firmable pierde su firmante.
-- Nota de datos: se PIERDEN las matrículas cargadas (columnas dropeadas).
-- ═══════════════════════════════════════════════════════════════════════════
...
DROP FUNCTION IF EXISTS public.vets_sin_matricula();
DROP FUNCTION IF EXISTS public._empleado_matricula_ok(uuid, text);
ALTER TABLE public.prestador_empleados
  DROP COLUMN IF EXISTS matricula_profesional,
  DROP COLUMN IF EXISTS matricula_pais_emisor;
