...
## El cuerpo del POST — pegar la tarjeta y ejecutar
...
🔴 **El PAN no se inventa.** El arnés rebota `falta_tarjeta_de_prueba` si no viene, a
propósito: un número plausible pasaría los formatos y fallaría en el cobro sin decir
por qué.
...
|---|---|---|
| `stoken_valido = true` | **fórmula y lugar confirmados** | se fija el lugar que dice `stoken_de`, se borran los otros dos candidatos, y el buzón pasa a **actuador** (transición idempotente vía `confirmar_pago_compra`, `p_monto` siempre poblado) |
