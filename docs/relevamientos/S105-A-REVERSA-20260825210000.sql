-- ══════════════════════════════════════════════════════════════════════════
-- REVERSA de 20260825210000_s105a_actuador_reconoce_reverso_nuvei.sql
-- Escrita ANTES de aplicar (regla de la casa).
--
-- QUÉ DESHACE: devuelve `aplicar_evento_de_pago` a su definición byte-idéntica
-- previa (volcada de `pg_get_functiondef` el 25-ago-2026) y retira la función
-- de vocabulario `_nuvei_status_detail_es_reverso`.
--
-- 🔴 QUÉ **NO** DESHACE — y es lo que hay que saber antes de correrla:
--   · Los intentos que hayan quedado en `reversado` POR ESTE CAMINO **se
--     quedan así**. Revertir el reconocedor no des-reversa nada: la plata ya
--     volvió del lado del proveedor y el registro dice la verdad.
--   · Los `webhook_events` marcados `aplicado` con detalle «reverso …»
--     conservan su marca. Volverían a leerse como `desconocido` sólo si
--     alguien los re-procesa, y eso sería PERDER información, no recuperarla.
--   · El sujeto (compra/pedido/cita) **nunca se movió** — ni antes ni después
--     de esta migración. Es `D-923` y sigue abierta.
--
-- CONSECUENCIA DE CORRERLA: los reversos vuelven a morir en `desconocido`,
-- confundidos con rechazos. Es exactamente el estado que esta migración cura.
-- ══════════════════════════════════════════════════════════════════════════
...
DROP FUNCTION IF EXISTS public._nuvei_status_detail_es_reverso(text);
