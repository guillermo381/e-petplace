...
BEGIN
  ...
  PERFORM registrar_intencion_notificacion('cita_recordatorio', v_u, v_m, NULL, '{}'::jsonb, 'lec-1');
  PERFORM registrar_intencion_notificacion('promocion',         v_u, v_m, NULL, '{}'::jsonb, 'lec-2');
END $$;
SELECT set_config('request.jwt.claims',
  json_build_object('sub',(SELECT ur.user_id FROM user_roles ur WHERE ur.role='admin' LIMIT 1),
                    'role','authenticated')::text, true);
SELECT que, canal, resultado, por_que, modo
  FROM leer_sombra_notificaciones(now() - interval '5 minutes', now());
ROLLBACK;
