-- ══════════════════════════════════════════════════════════════════════════
-- REVERSA de 20260826030000_s105a_mover_sujeto_por_reverso.sql
-- Escrita ANTES de aplicar.
--
-- QUÉ DESHACE: retira `mover_sujeto_por_reverso`, saca su llamada del actuador
-- (que vuelve byte-idéntico) y desactiva el tipo de aviso `pago_reversado`.
--
-- 🔴 QUÉ **NO** DESHACE, y es grave:
--   · Los sujetos ya cancelados por un reverso **NO vuelven a estar pagados**,
--     y es correcto: la plata volvió. *Revertir el código no revierte el banco.*
--   · **Vuelve la divergencia de `D-923`**: un reverso deja el intento en
--     `reversado` y la compra/cita diciendo que está pagada.
--   · Los avisos ya registrados quedan en la cola.
--
-- ⚠️ El tipo de aviso NO se borra —se desactiva— porque hay intenciones que lo
-- referencian: borrarlo dejaría filas apuntando a un código inexistente.
-- ══════════════════════════════════════════════════════════════════════════

UPDATE public.cat_notificacion_tipos SET activo = false WHERE codigo = 'pago_reversado';
DROP FUNCTION IF EXISTS public.mover_sujeto_por_reverso(uuid, text);
