-- REVERSA de 20260906100000_s109a_ancla_generica_y_llave_explicita.sql — ANTES.
-- ⚠️ Revertir devuelve el nombre `guarderia_proximo_cobro` y BORRA la fila de
--    `app_config`: el cron vuelve a asumir apagado POR AUSENCIA, que es
--    exactamente lo que esta migracion hace imposible.
BEGIN;
DROP FUNCTION IF EXISTS public.proximo_cobro_mensual(smallint, date);
DELETE FROM app_config WHERE clave='guarderia_recurrente_vivo';
CREATE OR REPLACE FUNCTION public.guarderia_proximo_cobro(p_dia_cobro smallint, p_periodo_desde date)
 RETURNS date
 LANGUAGE sql
 IMMUTABLE
