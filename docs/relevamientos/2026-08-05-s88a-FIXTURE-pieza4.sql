...
-- fixture
-- ════════════════════════════════════════════════════════════════════════════
-- FIXTURE DE ④ — los tres rojos producidos ANTES de creerle al verde (L-199).
-- in-txn, termina en ROLLBACK: nada queda.
-- ════════════════════════════════════════════════════════════════════════════
DO $$
DECLARE v_m uuid; v_u uuid; v_r jsonb; v_out text := '';
        v_id_vieja uuid; v_id_nueva uuid;
BEGIN
  SELECT m.id, fm.user_id INTO v_m, v_u FROM mascotas m
    JOIN familia_miembro fm ON fm.familia_id=m.familia_id
   WHERE m.estado_vida='activa' AND fm.hasta IS NULL LIMIT 1;

  -- ── VERDE 0: con todo encendido, una intención pasa y queda en sombra ────
  PERFORM registrar_intencion_notificacion('cita_recordatorio', v_u, v_m, NULL,'{}'::jsonb,'p4:normal');
  v_r := despachar_notificaciones(false);
  v_out := v_out || format('V0(todo encendido)=%s | ', v_r->>'sombra_habrian_salido');

  -- ── ROJO 1: APAGADO → nada "saldría", y el rastro lo dice ───────────────
  UPDATE notificacion_config SET despacho_activo=false, motivo='fixture: apagon de prueba',
         apagado_en=now() WHERE alcance='global';
  PERFORM registrar_intencion_notificacion('cita_recordatorio', v_u, v_m, NULL,'{}'::jsonb,'p4:apagon');
  v_r := despachar_notificaciones(false);
  v_out := v_out || format('R1(apagado): habrian_salido=%s retenidas=%s | ',
                           v_r->>'sombra_habrian_salido', v_r->>'retenidas');

  -- ── ROJO 2: EL FUSIBLE. Se re-enciende y se baja el techo a 0 ───────────
  UPDATE notificacion_config SET despacho_activo=true, motivo=NULL, techo_duro_max=0
   WHERE alcance='global';
  -- una entrega simulada dentro de la ventana, para que el fusible tenga qué contar
  UPDATE notificacion_intencion SET estado='entregada', updated_at=now()
   WHERE clave_dedup='p4:normal';
  PERFORM registrar_intencion_notificacion('cita_recordatorio', v_u, v_m, NULL,'{}'::jsonb,'p4:fusible');
  v_r := despachar_notificaciones(false);
  v_out := v_out || format('R2(fusible): por_techo=%s · config_quedo_apagada=%s · motivo="%s" | ',
    v_r->>'retenidas_por_techo',
    (SELECT NOT despacho_activo FROM notificacion_config WHERE alcance='global'),
    (SELECT left(motivo,34) FROM notificacion_config WHERE alcance='global'));

  -- ── EL PAR DEL RE-ENCENDIDO: uno que envejece y uno que no ──────────────
  UPDATE notificacion_config SET despacho_activo=true, motivo=NULL, techo_duro_max=500
   WHERE alcance='global';
  -- (a) una CITA de hace 48 h → `operacion` vence a las 24 h ⇒ debe MORIR
  v_id_vieja := registrar_intencion_notificacion('cita_recordatorio', v_u, v_m, NULL,'{}'::jsonb,'p4:vieja');
  UPDATE notificacion_intencion SET created_at = now() - interval '48 hours' WHERE id = v_id_vieja;
  -- (b) un aviso de CUENTA de hace 48 h → `seguridad_cuenta` NO envejece ⇒ SALE
  v_id_nueva := registrar_intencion_notificacion('sistema', v_u, v_m, NULL,'{}'::jsonb,'p4:nover');
  UPDATE notificacion_intencion SET created_at = now() - interval '48 hours' WHERE id = v_id_nueva;

  v_r := despachar_notificaciones(false);
  v_out := v_out || format('PAR: vencidas=%s · vieja=%s/%s · no-envejece=%s',
    v_r->>'vencidas_al_reevaluar',
    (SELECT estado FROM notificacion_intencion WHERE id=v_id_vieja),
    (SELECT motivo FROM notificacion_intencion WHERE id=v_id_vieja),
    (SELECT estado FROM notificacion_intencion WHERE id=v_id_nueva));

  PERFORM set_config('epp.p4', v_out, true);
END $$;
SELECT current_setting('epp.p4', true) AS fixture_pieza4;
ROLLBACK;
