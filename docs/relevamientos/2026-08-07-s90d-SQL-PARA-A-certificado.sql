-- ═══════════════════════════════════════════════════════════════════════════
-- S90-D · EL CERTIFICADO DE SALUD — SQL LITERAL COMPLETO PARA LA PISTA A
--
-- ⚠️ ESTA PISTA NO ESCRIBE DB (76b). Este archivo es el PEDIDO: A lo revisa,
-- lo mueve a `supabase/migrations/<timestamp>_s90d_certificado_salud.sql` y
-- lo aplica. El timestamp propuesto (20260807060000) deja aire por si A
-- escribió otras migraciones el mismo día — la numeración es de A.
--
-- ── POR QUÉ ESTE PAPEL PIDE MOTOR PROPIO Y LOS OTROS DOS NO ────────────────
-- El carnet y la historia clínica IMPRIMEN LO QUE YA HAY: su papel es una
-- vista de datos que otro acto sedimentó. El certificado NO — certifica un
-- JUICIO PROFESIONAL («esta mascota está apta») que hoy nadie captura en
-- ninguna tabla. Sin una tabla que lo guarde, el papel tendría que DERIVARLO,
-- y derivar «apto» de la ausencia de condiciones en el expediente es fabricar
-- una firma que nadie dio (un silencio clínico no es un diagnóstico).
--
-- ⇒ El certificado es un ACTO, no una vista. Por eso: tabla propia, snapshots
--   congelados, e inmutabilidad (corregir es EMITIR OTRO — familia D-544).
--
-- ── LO QUE SE CONSUME Y NO SE CLONA (§6 del método) ────────────────────────
-- · `empleado_tiene_capacidad_clinica(prestador, user)` — EL helper único del
--   gate clínico (flip §6.2, S76). No se re-implementa el predicado.
-- · `user_tiene_acceso_a_mascota(mascota)` — la MISMA puerta del expediente.
--   El papel NO ensancha permisos.
-- · `_user_opera_cuenta_comercial(cuenta, uid)` — para que el negocio emisor
--   relea lo suyo.
--
-- ── EL HELPER QUE **NO** SE CONSUME, Y ES UN HALLAZGO ──────────────────────
-- `_empleado_matricula_ok(empleado, tipo)` contesta **«¿puede recibir citas
-- médicas?»**, y por la GRACIA de D-676 devuelve TRUE para un empleado
-- pre-existente que TODAVÍA NO TIENE MATRÍCULA (hasta 2026-09-01).
-- Usarlo acá dejaría emitir un certificado con la línea de matrícula VACÍA —
-- exactamente lo que la orden prohíbe. **El helper de ELEGIBILIDAD no es el
-- helper de FIRMA.** Acá el gate es la PRESENCIA LITERAL del dato, sin gracia:
-- un certificado de aptitud firmado por «el negocio» no certifica nada, y por
-- eso tampoco hay fallback al negocio (la receta sí lo tiene; este papel no).
--
-- 76(g): NO RIGE — DDL aditivo + DROP/CREATE de una RPC, cero backfill.
-- L-140: toda función nueva nace con REVOKE anon/PUBLIC + GRANT explícito.
-- D-662: `emitir_token_documento` se DROPea y recrea (agregar un parámetro a
--        una función es una SOBRECARGA, no un reemplazo — L-119). El cinturón
--        verifica `sobrecargas = 1`. Los dos callers vivos (carnet, historia
--        clínica) siguen llamando con 2 args y siguen resolviendo.
-- REVERSA: docs/relevamientos/2026-08-07-s90d-REVERSA-certificado.sql
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- jamas un negocio. Y sin gracia — ver la nota de cabecera sobre por que
  -- `_empleado_matricula_ok` NO sirve para esto.
...
-- ───────────────────────────────────────────────────────────────────────────
-- ⑤ EL TOKEN APRENDE A NOMBRAR UN ACTO
-- ───────────────────────────────────────────────────────────────────────────
-- Los dos papeles vivos imprimen «todo lo de esta mascota» — les alcanza con
-- `mascota_id`. El certificado imprime UN acto: sin `referencia_id`, el papel
-- tendria que adivinar cual (el ultimo), y «el ultimo» miente en cuanto haya
-- dos. La columna nace NULL y los dos papeles vivos la ignoran.

ALTER TABLE public.documento_token ADD COLUMN referencia_id uuid;
COMMENT ON COLUMN public.documento_token.referencia_id IS
  'S90-D: cual ACTO imprime este token. NULL para los papeles que imprimen el expediente entero (carnet, historia clinica); OBLIGATORIO para el certificado.';
...
ALTER TABLE public.documento_token
  ...
  CHECK (tipo IN ('carnet_vacunas', 'historia_clinica', 'certificado_salud'));

-- Agregar un parametro es una SOBRECARGA, no un reemplazo (L-119) — y dos
-- sobrecargas visibles por PostgREST son una ambiguedad en produccion.
...
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'auth_required' USING ERRCODE = '42501';
  END IF;
  IF p_tipo IS NULL OR p_tipo NOT IN ('carnet_vacunas', 'historia_clinica', 'certificado_salud') THEN
  ...
  -- El certificado nombra UN acto, y ese acto tiene que ser DE ESTA MASCOTA:
  -- sin esta verificacion, un token valido para la mascota A podria imprimir
  -- el certificado de la mascota B.
  IF p_tipo = 'certificado_salud' THEN
    IF p_referencia_id IS NULL THEN
      RAISE EXCEPTION 'referencia_requerida' USING ERRCODE = '22023';
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM certificado_salud c
      WHERE c.id = p_referencia_id AND c.mascota_id = p_mascota_id
    ) THEN
      RAISE EXCEPTION 'referencia_no_es_de_la_mascota' USING ERRCODE = '22023';
    END IF;
  END IF;

  INSERT INTO documento_token (user_id, mascota_id, tipo, referencia_id, expira_en)
  VALUES (v_uid, p_mascota_id, p_tipo, p_referencia_id, now() + interval '10 minutes')
...
-- ───────────────────────────────────────────────────────────────────────────
-- ⑥ CINTURONES — todos se verifican contra el objeto, ninguno contra el texto
-- ───────────────────────────────────────────────────────────────────────────
DO $cint$
DECLARE v_n int; v_src text;
BEGIN
  -- (a) UNA sola sobrecarga de emitir_token_documento (L-119)
  ...
  -- (b) el CHECK admite el tipo nuevo
  IF pg_get_constraintdef((SELECT oid FROM pg_constraint WHERE conname = 'documento_token_tipo_check'))
     NOT LIKE '%certificado_salud%' THEN
    RAISE EXCEPTION 'cinturon_certificado: el CHECK no admite certificado_salud';
  END IF;
...
-- ───────────────────────────────────────────────────────────────────────────
-- ⑦ FIXTURE in-txn (ROLLBACK) — para que A lo corra ANTES de aplicar
-- ───────────────────────────────────────────────────────────────────────────
-- Corre COMO postgres y prueba los BRAZOS del motor, no la RLS (esa se prueba
-- con JWT real, y va aparte). Se ejecuta suelto, jamas dentro de la migracion.
--
--   BEGIN;
--     -- (1) el gate de la matricula REBOTA hoy: 0 empleados la tienen
--     --     esperado: matricula_profesional_faltante
--     -- (2) con matricula seteada in-txn, la emision PASA
--     -- (3) memorial: con estado_vida='fallecida', alcance='viaje' REBOTA
--     --     (alcance_no_aplica_al_estado) y alcance='constancia' PASA
--     -- (4) el token de certificado SIN referencia_id REBOTA
--     -- (5) el token con referencia de OTRA mascota REBOTA
--     -- (6) UPDATE sobre el certificado emitido REBOTA (inmutable)
--   ROLLBACK;
--
-- El discriminador del brazo (2) es el par: MISMOS datos, unica diferencia la
-- columna `matricula_profesional` — sin ella rebota, con ella emite.
