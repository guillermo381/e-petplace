...
- **`/invitacion?token=` y `/baja?t=` en `epetplace-web`** — bloquean el correo de invitación Y el enlace copiable (por eso el freno está apagado). `/invitacion` = landing estática (**C**). `/baja` necesita un **ENDPOINT** (edge function estilo `capturar-lead`; el sitio NO mete la anon key — política en `Formulario.astro`) → **A/D**. **Cierre: 200 medido con control positivo, jamás «está hecha».** Deploy = Vercel al mergear a main del sitio.
- **Pantallas de código** (`confirmarAltaConCodigo`, `CampoCodigo`, reenvío 60s, detrás del flag) — **C**. 🔴 el consentimiento se **MUESTRA en el formulario** y se **PERSISTE al confirmar el código** (para sobrevivir al flip del autoconfirm — D-893).
