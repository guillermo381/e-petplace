...
| **`MapaZona` afirma una salvaguarda que su código no tiene** — su cabecera dice `zona*` y las props se llaman `lat`/`lon`; con `MapaPunto` viva, la confusión es más fácil, no menos | **B** — `docs/loop/S109-D-PEDIDO-B.md` | las dos salidas servidas; **no elegí**: el modo de falla decide la herramienta (`L-396`) |
