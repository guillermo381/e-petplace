/* ═══════════════════════════════════════════════════════════════════════════
   REVERSA de 20260908960000_s113a_modo_captura_ia.sql
   Escrita ANTES de aplicar (regla de la casa). S113-A · lote 0.

   ⚠️ QUÉ **NO** DESHACE, dicho con todas las letras:
   · Los eventos que quedaron marcados con `modo_captura` NO vuelven a NULL.
     **Es a propósito**: `modo_captura` registra CÓMO se capturó un hecho, y
     ese hecho ocurrió. Revertir el código no cambia la historia. La línea
     que además borra las marcas está al pie, comentada, y NO corre sola.
   · No toca el CHECK `eventos_mascota_modo_captura_check` (pre-existente).
   · No toca `_crear_evento_padre_auto` ni `_crear_padre_constelacion`:
     **la migración tampoco los tocó** — precisamente para no hacer cirugía
     de firma sobre una puerta con 19 consumidores (L-119: un parámetro con
     DEFAULT no reemplaza, SOBRECARGA, y deja las llamadas ambiguas).
   · No toca `registrar_vacuna_mostrador` (el mostrador sigue sin declarar
     modo de captura — está declarado como fuera de alcance del lote).

   ⚠️ REVERTIR ES UN ACTO DE DOS LADOS: si ya se publicó bundle con
   `modoCaptura` en `sedimentarNotaClinica`, revertir la base sin revertir el
   bundle **rompe la sedimentación de la consulta** (la firma de 8 argumentos
   deja de existir). Se revierten juntos o no se revierte.
   ═══════════════════════════════════════════════════════════════════════ */

BEGIN;

-- ① El marcador desaparece. Con él, los dos caminos vuelven a dejar NULL.
DROP FUNCTION IF EXISTS public._marcar_modo_captura_evento(uuid[], text);

-- ② sedimentar_nota_clinica: muere la de 8 argumentos y REVIVE la de 7.
--    El cuerpo va embebido porque tras el DROP no queda ninguna definición
--    ejecutable en un solo archivo — la reversa tiene que poder correr sola.
DROP FUNCTION IF EXISTS public.sedimentar_nota_clinica(uuid, uuid, uuid, uuid, jsonb, jsonb, text, text);
...
-- ③ registrar_vacunas_de_carnet vuelve a NO marcar. Cuerpo previo embebido,
--    capturado del OBJETO (pg_get_functiondef) antes de aplicar la migración.
--    Misma firma ⇒ CREATE OR REPLACE, sin DROP y sin sobrecarga.
...
/* ── VERIFICACIÓN de que la reversa hizo lo suyo ──────────────────────────
   Correr después:
     ...
     -- esperado: 1  (la de 7 argumentos, y SOLO ella)
   ──────────────────────────────────────────────────────────────────────── */

/* ── acto APARTE, deliberado, que esta reversa NO ejecuta ─────────────────
UPDATE eventos_mascota SET modo_captura = NULL
 WHERE modo_captura IN ('extraido_por_ia','dictado','tecleado');
   ───────────────────────────────────────────────────────────────────── */
