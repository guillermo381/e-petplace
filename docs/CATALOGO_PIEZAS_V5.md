...
- 🔴 **LA ETIQUETA Y EL VALOR TIENEN CADA UNO SU RENGLÓN, Y ESO ES UN ALTO FIJO — NO UN REPARTO.** El interior mide **38 = etiqueta flotada (14) + línea (24)**, así que `center` no tiene nada que repartir. ⏪ **Defecto del founder en el aparato:** *«la etiqueta tapa la mitad del texto que se escribe»* — **medido: con la etiqueta arriba el input caía de 23 dp a 11.**
- 🔴 **Y la causa era un `flex: 1` que CAMBIÓ DE EJE sin cambiar de línea.** Hasta el lote 6 el input era hijo directo de la FILA y su `flex: 1` repartía el **ancho**; el lote 6 lo metió en una COLUMNA para alojar la etiqueta, y el mismo `flex: 1` pasó a repartir el **ALTO**. *El `flex` no se movió ni cambió de valor: le cambiaron el padre. Es la clase de defecto que no se ve leyendo el diff de la línea, porque la línea no está en el diff.* Ahora `flex` sólo vive en multilínea, que es donde el alto sí se reparte.
- 📏 **Verificado en píxeles en las tres pantallas, y en ninguna se solapan:** 05 → etiqueta 863–900 · valor 900–969 (23,0 dp) · **alta** → 650–687 · 687–757 (23,3 dp) · **`CampoFecha`** → 1264–1301 · 1301–1370 (23,0 dp).
