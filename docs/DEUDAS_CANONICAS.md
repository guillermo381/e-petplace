...
#### D-523 — Voseo en `veterinaria-nota-clinica.ts` (excluido del barrido a propósito — camino del gate) ⚪
⚪ MÍNIMA. Los strings de error de `packages/api/src/wrappers/veterinaria-nota-clinica.ts` están en **voseo** (relevado S75): *"No **tenés** acceso a esta mascota"* (`:179`, `:424`, `:472`), *"No **sos** la clínica tratante de este caso"* (`:191`). La casa habla **tuteo neutro** (regla 27, L-148). **Se excluyó del barrido de voz a propósito** — es el camino del gate del founder (la nota clínica), y tocarlo mientras el gate corría arriesgaba el flujo. **La cura es mecánica** (tenés→tienes, sos→eres, en la voz del diccionario). **Disparo: el próximo lote de strings del path vet, o la pasada de voz del prestador.** Origen: S75 (relevado en el diagnóstico A25).
