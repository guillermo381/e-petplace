...
> **ENMIENDA S76-A2 (la cura, RE-PROPUESTA — hallazgo del founder sobre el primer diff):** el trigger propuesto guardaba `activo` y `rol`, pero **`prestador_id` TAMBIÉN es escribible por el propio empleado** (`empleados_self_actualiza` no lo cubre ni en USING ni en WITH CHECK) — y **MOVERLO logra el mismo fin sin tocar columna guardada: fila ACTIVA en negocio AJENO.** Dos correcciones al diff: (1) `prestador_id` entra al IF de columnas guardadas; (2) la titularidad se evalúa contra **`OLD.prestador_id`** (no `NEW`) — hay que proteger el negocio del que la fila SALE. La verificación gana un tercer check (el empleado intenta mover `prestador_id` → rebota 42501). **RE-PROPUESTA, NO APLICADA (regla 73).** **Y cuando A2 quede aplicada: D-526 NO se marca CERRADA** — la cura tapa la escritura DIRECTA (`current_user='authenticated'`); **el censo de los DEFINERs que escriben `prestador_empleados` queda ABIERTO (D-528), porque el trigger no los ve** (L-153: un agujero tapado por un lado no es un agujero cerrado; L-167 aplicada a la cura).
