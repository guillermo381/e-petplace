...
**Enmienda S46 — barrido PARCIAL:** el founder borró a mano del dashboard los objetos con sello `avatar-diag-` (huérfanos inequívocos por nombre). PENDIENTE: cruce DB de los dudosos (`avatar-grande-` y cualquier otro sin sello) + la vía DELETE gateada (sigue atada a D-303, sin cambio). Disparo ampliado: el prompt S46-B0-D301 quedó redactado y NO ejecutado — es arranque de S47 (junto a D-310, que salió de la misma captura del dashboard).
...
🟢 MEDIA. La RPC `registrar_vacunas_de_carnet` no acepta `archivo_url` y la function no guarda la imagen — conservar el carnet como evidencia es espec pendiente; emparenta con la estrategia de limpieza del bucket (D-301/D-303). Disparo: espec de la UI del flujo carnet. Origen: S46-B1.1.

#### D-309 — `crear_cliente_walkin` desplegada sin fuente en repo
🟢 MEDIA. Edge Function ACTIVE v3, `verify_jwt=true`, con el código ausente de los 5 directorios del ecosistema (solo referencias documentales de S7 prestadores). Recuperable con `supabase functions download`. Principio: toda function viva tiene su fuente versionada en el monorepo (`extract-vacuna` cumple desde S46; `chat-ayuda` sigue solo en v2, repo archivado pero en disco). Disparo: próxima sesión que toque Edge Functions. Origen: S46-B1.0.

#### D-310 — Bucket `mascotas` PUBLIC + 2 policies SELECT amplias
🟡 ALTA. Warning del dashboard (captura founder S46): el bucket es PUBLIC y hay 2 policies SELECT amplias — posible exposición de fotos más allá del diseño (¿la policy S45 `cita_archivos_storage_select_acceso_mascota` queda decorativa?). El relevamiento está redactado en el prompt S46-B0-D301 (partes 1-3), NO ejecutado. Disparo: arranque S47 — y resolución ANTES de dueños reales. Origen: S46 (captura del dashboard).
