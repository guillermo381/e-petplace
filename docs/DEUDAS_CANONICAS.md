...
🟠 MEDIA-ALTA. **Hallazgo del cruce A7 (S75), con R1 ya abriendo la puerta.** R2 resuelve la cuenta comercial por `owner_profile_id` (owner-only); a un empleado no-owner le devuelve `data:null`. Con R1 (S75-A1) dejando entrar a los empleados activos, las **19 consumidoras de R2** pasan a ser alcanzables por ellos, y los gates de B2 (`a764563`) cubren **solo NEGOCIO + los 4 talleres** — el resto queda descubierto. **≥6 pantallas ROMPEN** (pantalla de error) para un no-gestor, fuera de NEGOCIO: `veterinaria/mostrador/autorizar.tsx` (**el "Registrar atención" de la RECEPCIÓN — el corazón del E2E**), `veterinaria/consulta/[citaId]`, `veterinaria/coordinar/[citaId]`, `veterinaria/movimiento`, `cuenta-comercial/index`, `cuenta-comercial/bancarios`. Otras degradan honestas (HOY, CUENTA, `cita/[citaId]`, `presupuesto/nuevo`). **La cura correcta NO es gatear el mostrador** (la recepción DEBE recibir, A3.4): es que el mostrador resuelva su `cuenta_comercial_id` por el NEGOCIO del empleado — **R1 ya lo trae** (`cuenta_comercial_id` entró a `MiPrestador` en A1) — en vez de por owner. Es la migración R2→R1 de las consumidoras de atención. Censo con las rutas y las líneas exactas: `2026-07-23-s75a-censo-r2-alcance-empleado.md`. **Es el PRIMER ÍTEM del arco de equipo v2** (R2 quedó fuera del v1 por decisión founder). **Disparo: el arco de equipo v2 / el primer E2E de recepción completo.** Origen: S75-A7 (cruce, antes del publish).
...
> **ENMIENDA S75-A (la corrección que la sesión se debe a sí misma) — LA PUERTA NO ES UN ARCHIVO, ES EL RESOLVEDOR.** El corolario original nombraba la puerta como *"`(tabs)/_layout.tsx:63-65` + el resolvedor R1"* — dos cosas, y el brief numeró el ARCHIVO (`_layout.tsx`, ítem 6) como el interruptor cuando el interruptor real era el RESOLVEDOR (R1, ítem 4). **Verificado con literal en el cruce A6 (S75): con R1 en HEAD, el guard de `_layout.tsx:78-79` (`const p = await obtenerMiPrestador(); if (p.ok) return { ok:true }`) deja pasar a cualquier empleado activo — el interruptor se apretó con el commit de un WRAPPER (`3591db2`), sin que nadie llamara a "abrir la puerta".** El archivo del guard nunca fue la barrera: la barrera era a quién resuelve el wrapper. Corolario corregido: **el orden en piedra nombra el ARTEFACTO que abre (el resolvedor), jamás el archivo donde se lo espera (el guard).**
