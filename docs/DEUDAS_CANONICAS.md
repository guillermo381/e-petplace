...
**LA CORRECCIÓN DE ENCUADRE (mesa S74, con estas palabras):** **abrir la puerta sin los gates NO expone LECTURA — la RLS la cubre, incluido lo clínico. ENTREGA ESCRITURA.** La pregunta de mesa (*"¿UX rota o fuga de datos?"*) **tenía una TERCERA respuesta que ninguna de las dos sesiones podía ver sola**: A midió el resolvedor, B midió la puerta, y **el cruce produjo este hallazgo**. Los dos indivisibles son **D-490** (escritura clínica sin rol) y **D-513** (gestión de NEGOCIO sin rol administrador: servicios, horarios y precios editables por cualquier empleado).

**LA PUERTA VA ÚLTIMA (regla de orden del arco, destapada por el censo de B — al canon):** **el resolvedor, los gates por rol y las voces se pueden construir EN CUALQUIER ORDEN y quedan INERTES mientras la puerta siga cerrada; abrirla es el INTERRUPTOR que enciende el arco completo.** Consecuencia práctica que ordena la decisión del founder: **declarar el arco desconectado NO pierde trabajo — lo deja armado esperando el switch.** **Corolario operativo, en su forma final: LA PUERTA VA ÚLTIMA, y ANTES DE LA PUERTA VAN D-490 Y D-513. No es preferencia: es la CONDICIÓN para que abrirla no entregue la lapicera.** La puerta es `(tabs)/_layout.tsx:63-65` + el resolvedor R1.
...
#### D-513 — EL GATE DE GESTIÓN DE NEGOCIO POR ROL ADMINISTRADOR (el segundo indivisible) 🔴
🔴 ALTA. **El hermano sin número de D-490, hallado en el mismo cruce (S74-A) y ahora registrado.** **Servicios, horarios y PRECIOS del taller cuelgan de R1** (`obtenerMiPrestador`) — o sea que **el día que el resolvedor acepte el vínculo de empleado, cualquier empleado activo podría VER y EDITAR la configuración comercial del negocio**, sin ser administrador. **Choca de frente con la letra FIRMADA de roles** (`LETRA_ROLES_EQUIPO_S74` §2: *plata · gestión · equipo* son del **administrador**; el profesional tiene chips de oficio y recepción es el piso) y con `DISEÑO_EXPERIENCIA` §15b (NEGOCIO se gatea por rol, HOY no). **Alcance:** las superficies de NEGOCIO que configuran la oferta (talleres de oficio, horarios, precios, modo de horarios) ganan gate **`empleado_tiene_rol(negocio, ['dueño','administrador'])`** — mismo helper, mismo patrón de AUSENCIA (Ley 23) que B usó en la celda de Equipo; **la plata NO necesita gate nuevo** (su RLS es owner-only, verificado).
**POR QUÉ EL ARRAY LLEVA LOS DOS (precisión de mesa S74 — un gate solo-`dueño` sería correcto HOY y equivocado el día que importa):** la letra FIRMADA hoy dice que **plata · gestión · equipo son del ADMINISTRADOR**, y **`administrador` NO EXISTE todavía como valor de rol** (es uno de los pedidos de motor de B). **D-513 es precondición de abrir la puerta, y el día que la puerta se abra `administrador` ya existe** — un gate que mire solo `dueño` **dejaría afuera exactamente al rol para el que fue creado**. Por eso nace con los dos valores desde el principio: hoy `dueño` es el único que existe (y el literal probó que **el dueño del backfill ES el titular, 5/5**), mañana `administrador` entra sin tocar el gate.
**⚠️ PREGUNTA ABIERTA DE VOCABULARIO — para la LETRA DE MOTOR de S75, declarada acá porque hoy no vive en ningún lado y S75 la va a necesitar:** en el **MOTOR** el valor es **`dueño`**; en la **LETRA** el nivel asignable nuevo se llama **`administrador`**, y **el titular NO es asignable** (no aparece en el selector, §4 de la letra de roles). **¿`dueño` se RENOMBRA a administrador · CONVIVEN los dos valores · o `administrador` nace como valor APARTE?** **NO se decide acá — la resuelve la mesa con el literal a la vista** (el CHECK vigente de `empleado_roles` admite `dueño|profesional|recepcion`, así que cualquiera de los tres caminos toca el CHECK). *Se declara por la misma clase de deriva que B atrapó con el 72 h de su boceto contra las 48 h firmadas: una decisión de mesa que no llega a un doc es una verdad que la sesión siguiente lee mal.* **Es INDIVISIBLE del resolvedor** junto con D-490: ver la regla de orden en D-512. **Disparo: la misma ventana que cure R1 — jamás después.** Origen: S74-A (cruce A×B sobre el censo de la puerta).
