...
#### D-518 — La reserva NO filtra la oferta por el país del cliente — en los CUATRO oficios 🟠
🟠 MEDIA (🔴 al soft launch multi-país). **Hallazgo de A18 (S75), verificado con literal en los 4 oficios.** Los RPC de disponibles (el paso "QUIÉN" de la reserva, la autoridad de qué se puede reservar) devuelven `ciudad` como **columna de display** (`RETURNS TABLE(… direccion text, ciudad text)`), pero **ninguno FILTRA por el país/ciudad del cliente**: sus WHERE gatean por tipo de servicio, disponibilidad, talla/elegibilidad de la mascota y `cc.estado='activa'` — cero geografía. Literal por oficio: `obtener_veterinarios_disponibles` (via `_vet_ofertas_cobrables`: WHERE `m.id = p_mascota_id` + estado/activo/reservable) · `obtener_groomers_disponibles` / `_grooming_ofertas_cobrables` (WHERE tipo + talla) · `obtener_adiestradores_disponibles` (WHERE `_mascota_elegible_servicio`) · `obtener_paseadores_disponibles` (**ni siquiera menciona `ciudad`** — SIN geo). **El LLAMADOR tampoco filtra:** el wrapper `obtenerVeterinariosDisponibles` (`veterinaria-reserva.ts`) recibe `{fecha, hora, tipo_servicio, mascota_id}` — sin país — y la pantalla `explorar/veterinaria/disponibles.tsx:65` muestra lo que el RPC devuelve. **Ningún piso (RPC · wrapper · pantalla) restringe por geografía.** **Es de la CASA, no del vet solo.** Hoy INVISIBLE (los 5 prestadores son EC y `guillo381+8` es EC), pero el día que exista un prestador no-EC, **un cliente EC vería sus ofertas en la cara** — y el canon declara la infra `country_config` viva desde S51 (el rumbo es multi-país; `prestador_zonas`/`cat_ciudades` existen desde S58-D-331 pero el motor de disponibles NO las lee — D-367 declaró "el motor NO filtra"). **NO se cura en S75 (no se abre frente nuevo): se declara.** **Disparo: ANTES del soft launch** (es multi-país por diseño) — o el primer prestador no-EC, lo que llegue primero. Origen: S75-A18 (subcuenta del censo A15 del negocio del gate).
