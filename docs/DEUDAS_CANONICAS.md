...
#### D-441 — `user_puede_ver_dimension` no reconoce al titular por familia_miembro (gap latente del expediente)
🟡 MEDIA — **a la mesa (decisión de privacidad, no cura en silencio).** El barrido de S69-A7 (patrón codueño/autorizado-sin-familia_miembro) halló una TERCERA función con el patrón: `user_puede_ver_dimension` (visibilidad del BIO_EXPEDIENTE). A diferencia de aprobar/rechazar (gate binario, ya curado con `_user_es_familia_de_mascota`), esta tiene otra semántica: **fast-path de acceso total para codueño/autorizado, luego level-gating por dimensión** para el resto. Un titular vinculado SÓLO por `familia_miembro` (sin fila en `mascota_codueño` — el caso de Thor) **no toma el fast-path** → vería el expediente sólo al nivel público/comunidad de cada dimensión, no como dueño pleno. Agregar la pata `familia_miembro` al fast-path sería coherente (el titular ES el dueño), pero **cambia el modelo de privacidad del expediente** — se decide en mesa, no se toca en una cura de gate. **Disparo: la sesión de privacidad del expediente, o si el founder confirma que quiere el fast-path por familia_miembro.** Origen: S69-A7 (barrido L-144).

#### D-440 — El prestador no puede RELEER un presupuesto (hueco de espec, no bug)
🟡 MEDIA. Del lado prestador la B construyó el armado (`nuevo.tsx`) + la entrada desde el detalle de cita, pero **no hay lector/pantalla de listado ni de relectura** de los presupuestos que emitió — "no puedo entrar de nuevo a verlo". Es hueco de ESPEC (el pedido B3 decía armar y enviar, no releer), no un defecto del guardado (las filas existen y se ven del lado dueño tras S69-A6). Necesita: lector de presupuestos del prestador (por cuenta/cita) + su pantalla de listado/detalle. **Disparo: la próxima tanda chica del prestador (o antes si el founder lo pide).** Territorio prestador (B). Origen: S69 (hallazgo de gate del founder).
