...
🟢 MEDIA. El badge "Prestador fundador" no es un sticker: es un PROGRAMA que necesita su letra — quién lo porta (¿los del grupo curado pre-soft-launch?), qué promete (¿visibilidad, condiciones, permanencia?), cuándo se otorga y si expira. La bienvenida del prestador (S61-B8, grupo curado) es la superficie natural que lo espera. **Disparo: la sesión del programa de fundadores, o antes de abrir a prestadores reales.** Origen: S61-B8 (asentado por la A en A14).

#### D-399 — El WhatsApp del founder como canal de solicitud de acceso
🟡 ALTA. La pantalla /solicitar-acceso del prestador (S61-B8.1) declara la verdad del grupo curado y ESPERA el contacto — el canal v1 es el WhatsApp del founder, con su letra §3.5 y **disparo de RETIRO explícito: cuando el flujo real de solicitud exista (o el volumen lo vuelva insostenible), el canal personal se retira — un número personal en producción no es un canal de producto.** El contacto es DATO del founder (L-139: jamás un número inventado en el repo). Origen: S61-B13 (pedido founder; su commit no había viajado a git al asentar esto — declarado). 
