...
#### D-444 — Ícono del presupuesto en lenguaje b′
🟢 BAJA. El presupuesto usa hoy el glifo **`pagos` como provisional vivo**. Le falta su propio ícono en el lenguaje b′ (DIRECCION_ARTE: objeto del oficio en trazo + la huella en el hex de su capa), con **gate founder POR ÍCONO a 21px** como todo el set. **Disparo: el lote siguiente de íconos, o el primer pedido founder.** Origen: S70.

#### D-445 — Centro de notificaciones (letra PENDIENTE + construcción)
🟡 MEDIA. **Doble deuda, y una es de esta sesión:** (1) **la LETRA CORTA del centro de notificaciones no se escribió — compromiso asumido en S70 y NO cumplido; lo hereda S71**; (2) la **construcción**, en tanda propia. Hoy las notificaciones se escriben (`notificaciones`, `rol_destino` `pet_parent`/`prestador`) pero **no tienen superficie de lectura**: el dueño se entera por push o por el badge contextual del Hogar, y no hay dónde ver el historial. La letra debe decidir: qué agrupa, qué se marca leído, qué caduca, y si el badge del Hogar es su única boca hasta que exista. **Disparo: S71 (letra) + la tanda de notificaciones (construcción).** Origen: S70.
...
🟡 MEDIA. **NÚMERO RESERVADO — el texto canónico es de la SESIÓN B y se deposita VERBATIM cuando llegue** (L-149: la mesa no presume contenido ajeno; se pidió y no viajó al depositar). Lo que esta fila SÍ puede declarar, por ser hallazgo propio de la vara cruzada A→B: el trabajo *"revelar el resto de una sección"* ya existe en el cliente **dos veces con dos vestidos distintos** (`citas/[mascotaId].tsx:400` `Boton secundario bloque` · `hogar/paseos.tsx:489` `Boton compacto sm`), así que un tercero local sería la tercera copia y **dispara Ley 11**. La paginación con cursor de `LineaDeVida` **no cuenta** (otro trabajo). **Disparo: tercer consumidor real.** Origen: S71 (vara cruzada del boceto B1).

#### D-455 — El nombre del negocio no le llega al dueño
