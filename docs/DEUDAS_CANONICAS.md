...
>
> **En el CIRCUITO de la cita agendada del founder rompe SOLO `consulta/[citaId]` (CLASE 1)** — la pantalla donde el prestador atiende la consulta reservada; `autorizar` es walk-in, `coordinar` es para por_coordinar (founder tiene 0), y las CLASE 2 están gateadas de facto. **La cura NO es gatear** (la recepción debe recibir, A3.4; el vet debe atender): es el **swap R2→R1** de las CLASE 1. Censo con rutas, líneas y camino: `2026-07-23-s75a-censo-r2-alcance-empleado.md`. **El alcance del OTA lo fija UNA pregunta: ¿el gate del founder llega a que el empleado ATIENDA, o se detiene en el handshake?** Si se detiene, el OTA sale y las CLASE 1 se declaran acá; si el empleado atiende, se curan las CLASE 1 antes (barato). **Primer ítem del arco de equipo v2** (R2 quedó fuera del v1 por decisión founder). Origen: S75-A7 (cruce, antes del publish).
