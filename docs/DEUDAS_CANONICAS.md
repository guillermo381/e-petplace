...
- **L-166** — **TODO DATO VIVO SE LEE AL MOMENTO DE USARLO, JAMÁS DE UN REPORTE ANTERIOR — por propio que sea el reporte.** Group, ancla, árbol, censo, predicción, y la afirmación del propio canon: la fuente se relee al momento del acto. **Absorbe las candidatas (e) y (f) y el principio del marcador de D-520** (todas marcadas absorbidas, no borradas). Cubre DOS clases de dato, y el segundo hallazgo es lo que la ensancha: **(1) datos operativos que envejecen** — los cinco casos de S75: el *group* leído de memoria · el *ancla* leída de un reporte previo (A20) · el *marcador* en pantalla contra un canon viejo (D-520) · la *predicción* de Ponte al día contra un censo viejo (A26, el founder reservó dos citas DESPUÉS de que corrió el censo) · la *carrera del ancla* (se lee al bundlear, no de un turno anterior); **(2) afirmaciones del canon que nunca fueron ciertas** — los dos de S76: la conclusión de D-522 era FALSA contra su propio predicado citado, y el "0 lectores" de `prestador_empleado_servicios` eran OCHO. Los de la clase (1) fueron verdad y dejaron de serlo; los de la clase (2) nunca lo fueron — la ley cubre las dos, porque la cura es la misma: leer la fuente al usarla, no heredar el dato. La disciplina ya existía dispersa (`eas update:list` para el group, `git rev-parse` al bundlear, `information_schema`/`pg_get_functiondef` para el schema); esta ley las unifica. Origen: S75 (cinco casos) + S76-A0/A0bis (D-522, el censo de chips). Firmada founder S76.
