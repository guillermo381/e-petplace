...
**LA PUERTA VA ÚLTIMA (regla de orden del arco, destapada por el censo de B — al canon):** **el resolvedor, los gates por rol y las voces se pueden construir EN CUALQUIER ORDEN y quedan INERTES mientras la puerta siga cerrada; abrirla es el INTERRUPTOR que enciende el arco completo.** Consecuencia práctica que ordena la decisión del founder: **declarar el arco desconectado NO pierde trabajo — lo deja armado esperando el switch.** Corolario operativo: la puerta (`(tabs)/_layout.tsx:63-65` + el resolvedor R1) es lo ÚLTIMO que se toca, y el día que se toque, D-490 y el gate de NEGOCIO tienen que estar puestos.
