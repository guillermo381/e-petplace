...
export function EspejoOferta({ datos }: { datos: DatosEspejoOferta }) {
  ...
  if (datos.duraciones.length > 0 && datos.desde !== null) {
    ...
    if (datos.conPlan && datos.conPaquete) lineas.push(t('ofertaPaseo.espejoConPlanYPaquete'));
    else if (datos.conPlan) lineas.push(t('ofertaPaseo.espejoConPlan'));
    else if (datos.conPaquete) lineas.push(t('ofertaPaseo.espejoConPaquete'));
