...
/** El orden del trabajo — la firma de la ventana. Menor = más arriba. */
export function prioridad(estadoInterno: string | undefined, narrativa: string): number {
...
export function VentanaPedidos({
  ...
  const { activos, terminados } = useMemo(() => {
    ...
    activos.sort((a, b) => {
      const pa = prioridad(extras[a.pedido_id]?.estado, a.narrativa);
      const pb = prioridad(extras[b.pedido_id]?.estado, b.narrativa);
  ...
  const huerfanos = useMemo(() => {
    ...
    xs.sort(
      (a, b) =>
        prioridad(extras[a.pedido_id]?.estado, a.narrativa) -
        prioridad(extras[b.pedido_id]?.estado, b.narrativa),
    );
    return xs;
  }, [extras, sinFecha]);
