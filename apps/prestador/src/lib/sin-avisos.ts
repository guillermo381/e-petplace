 ...
 *
 * **PORT DECLARADO desde `apps/cliente/src/lib/sin-avisos.ts`** — la misma
 * pieza al revés, igual que `permiso-push` en su día. *No se importa cruzado:
 * cada app tiene su `permiso-push`, y este módulo cuelga del suyo.*
 *
 * Del lado del prestador el costo es otro y peor: **un prestador sin avisos no
 * se entera de que se le abrió un caso, y tiene 24 h para responder.** No es
 * que se pierda una notificación: es que se le vence un plazo mientras no sabe
 * que existe.
 *
