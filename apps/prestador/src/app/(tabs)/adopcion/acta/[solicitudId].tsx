/**
 * EL ACTA Y LA FIRMA — lado REFUGIO (§4.2 «Firmar»).
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * **VOZ DEL FOUNDER:** *«La misma pantalla del acta que ve la familia, con mi
 * firma: código al correo de la cuenta. Estado: "Firmaste · falta la firma de
 * la familia".»*
 *
 * 🔴 **ES LA MISMA MECÁNICA Y NO LA MISMA PANTALLA, y la diferencia está
 * medida:** el motor decide el papel (`miPapel`), el estado de firmas se lee al
 * revés («falta la familia», no «falta el refugio»), la vuelta es al hilo del
 * portal y **acá NO hay hito**: el hito «Una vida nueva empieza» es del
 * expediente de la familia. *Celebrarlo del lado del refugio pondría el final
 * feliz en la casa de quien se despide.*
 *
 * ⚠️ **Lo que sí es idéntico vive en `DocumentoLegalLectura`**, la pieza de B —
 * el texto, el scroll y la cuenta de «vio todo». Lo que queda en cada archivo es
...
export default function ActaDelRefugio() {
  ...
  const pedirCodigo = async (acta: ActaAdopcion) => {
    ...
    try {
      ...
      mostrar({ variante: 'neutro', texto: t('actaRefugio.codigoEnviado', { a: r.data.enviadoA }) });
    } finally {
      setPidiendo(false);
    }
    void acta;
  ...
  const cuerpo = (acta: ActaAdopcion) => {
    ...
    const razon =
      pendientes.length > 0
        ? t('actaRefugio.faltaCargar')
        : !vioTodo
          ? t('actaRefugio.faltaLeer')
          : undefined;
        ...
        estadoFirmas={
          acta.firmas.length > 0 ? (
            <Texto variante="apoyo" color="tertiary">
              {t('actaRefugio.estadoFirmas', { n: acta.firmas.length })}
            </Texto>
          ) : undefined
        }
            ...
            <View style={{ gap: spacing[3] }}>
              {acta.faltantes.map((n) =>
                (RESOLUBLES as readonly string[]).includes(n) ? (
                  <Campo
                    key={n}
                    label={t(`acta.falta_${n}` as 'actaRefugio.falta_cedula')}
          ...
          <>
            {enviadoA === null ? (
              <Boton
                variante="primario"
                bloque
                etiqueta={t('actaRefugio.pedirCodigo')}
              ...
              <>
                ...
                <Boton
                  variante="primario"
                  bloque
                  etiqueta={t('actaRefugio.firmar')}
                  deshabilitado={codigo.length < 8}
                  razonDeshabilitado={t('actaRefugio.faltaCodigo')}
                  cargando={firmando}
                  onPress={() => void firmar()}
                />
              </>
    ...
    <View style={{ flex: 1, backgroundColor: theme.bg.base }}>
      ...
      {estado.fase === 'cargando' ? (
        ...
        <View style={{ flex: 1, justifyContent: 'center', padding: spacing[5] }}>
          <EstadoVacio
            titulo={t('actaRefugio.errorTitulo')}
            descripcion={t('actaRefugio.errorDetalle')}
            accion={
              <Boton
                variante="secundario"
                etiqueta={t('actaRefugio.reintentar')}
        ...
        /* 🔴 **SIN HITO, Y ES UNA DECISIÓN.** «Una vida nueva empieza» es del
           expediente de la FAMILIA — lo escribió la segunda firma en su misma
           transacción. *Celebrarlo acá pondría el final feliz en la casa de
           quien se despide.* El refugio ve que se completó, con su folio. */
        <View style={{ flex: 1, justifyContent: 'center', padding: spacing[5], gap: spacing[4] }}>
          <Texto variante="titulo">{t('actaRefugio.completaTitulo')}</Texto>
          <Texto variante="cuerpo">{t('actaRefugio.completaCuerpo')}</Texto>
          <Texto variante="apoyo" color="tertiary">
            {t('actaRefugio.folio', { folio: estado.folio })}
          </Texto>
          <Boton
            variante="primario"
            bloque
            etiqueta={t('actaRefugio.volverAlPortal')}
            onPress={() => router.replace('/adopcion')}
          />
        </View>
      ) : (
        cuerpo(estado.acta)
