 ...
 *
 * MATERIALES PENDIENTES DECLARADOS (armado en estructura, L-143: el
 * COPIAR NIVEL fino llega con el PNG patrón + los materiales de la A):
 *   · slider $0.25 con tick háptico → hoy Campo decimal (patrón aprobado
 *     S55-B2); swap directo cuando el componente nazca en packages/ui.
 *   · stepper del cupo → hoy SelectorOpcion 1-4 (patrón aprobado).
 *   · interruptor (toggle) → hoy Boton ghost Ofrecer/Pausar (patrón
 *     aprobado). · acento teal profundo → espera su token (B1c).
 * ZONAS DE COBERTURA: la sección NO nace (contrato de la A pendiente,
 * Ley 13 — jamás un lugar muerto).
 *
 * Letra leída (regla de piedra): MODELO_FINANCIERO v2.7 §7.13/§7.15
 * (neto SIEMPRE de fee_configs vía fees.ts) · MODELO_PASEO v1.4 §2
 * (menú canónico en CHECK de DB — BLOQUES_PASEO es su espejo).
...
export default function TallerPaseo() {
  ...
  const [errorPrecio, setErrorPrecio] = useState<Partial<Record<BloquePaseo, string>>>({});
  ...
  const listo = pantalla.estado === 'listo' && drafts !== null && franjas !== null;
  ...
  const hayCambios = useMemo(() => {
    ...
    return BLOQUES_PASEO.some((b) => ofertaDirty(drafts[b])) || franjas.some(franjaDirty);
  }, [drafts, franjas]);
  ...
  async function guardarTodo() {
    if (guardando || drafts === null || franjas === null || pantalla.estado !== 'listo') return;

    // validación previa con voz por duración
    for (const b of BLOQUES_PASEO) {
      const d = drafts[b];
      if (!ofertaDirty(d)) continue;
      if (d.ofrecida && leerPrecio(d.precio) === null) {
        setErrorPrecio((e) => ({ ...e, [b]: t('servicios.precioInvalido') }));
        setDuracionSel(b);
    ...
    <View style={{ flex: 1, backgroundColor: theme.bg.base }}>
      ...
      {listo && d !== null && (
        ...
        >
                ...
                {!d.ofrecida && d.base === null ? (
                  <>
                    ...
                    <Boton
                      ...
                      bloque
                      onPress={() => actualizarDraft(duracionSel, { ofrecida: true })}
                    />
                  </>
                ) : (
                  <>
                    ...
                    <Campo
                      ...
                      value={d.precio}
                      onChangeText={(v) => {
                        actualizarDraft(duracionSel, { precio: v });
                        setErrorPrecio((e) => ({ ...e, [duracionSel]: undefined }));
                      }}
                      keyboardType="decimal-pad"
                      ayuda={t('servicios.precioAyuda')}
                      error={errorPrecio[duracionSel]}
          ...
          {/* ZONAS DE COBERTURA: NO nace — contrato de la A pendiente (Ley 13) */}
