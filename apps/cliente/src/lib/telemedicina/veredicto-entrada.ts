...
const CLAVE_POR_MOTIVO: Record<MotivoEntrada, string | null> = {
  fuera_de_ventana: 'veterinaria.entrarFueraDeVentana',
  cita_no_pagada: 'veterinaria.entrarNoPagada',
  cita_cancelada: 'veterinaria.entrarCancelada',
  cita_no_realizable: 'veterinaria.entrarNoRealizable',
  cita_finalizada: 'veterinaria.entrarFinalizada',
  cita_inexistente: 'veterinaria.entrarInexistente',
  no_se_pudo_consultar: 'veterinaria.entrarNoSePudoConsultar',
...
export function queDibujar(v: VeredictoEntrada, idioma: string): QueDibujar {
  ...
  if (v.motivo === 'fuera_de_ventana') {
    ...
    if (hora === null) {
      return { boton: false, claveVoz: 'veterinaria.entrarNoSePudoConsultar' };
    }
