 ...
 * · **No cierra ni acepta.** `cerrarSolicitudAdopcion` es del PUBLICADOR —
 *   §10.2: *la plataforma jamás asigna, aprueba ni puntúa adoptantes*, y el
 *   adoptante tampoco se acepta a sí mismo.
...
export default function HiloSolicitud() {
  ...
  /** El estado a su familia de `Insignia`. **`declinada` va en `atencion`, no en
   *  `danger`:** §10.6 — *la devolución jamás humilla*, y una respuesta que no
   *  era la esperada no es un error del que la recibió. */
  ...
  const vozEstado = (e: EstadoSolicitudAdopcion): string =>
    t(`hiloAdopcion.estado_${e}` as 'hiloAdopcion.estado_recibida');
          ...
          >
            ...
            <Tarjeta relleno="normal" elevacion="reposo">
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[3] }}>
                ...
                <Insignia
                  estado={familiaDe(estado.hilo.estado)}
                  etiqueta={vozEstado(estado.hilo.estado)}
                />
              </View>
            </Tarjeta>
