 ...
 * detectaba productor-por-dato SÓLO por `cat_guarderia_transiciones`) y el
 * censo de E (que barre TODAS las columnas de texto de public + discriminador
 * causal)—. Este archivo ES el único; `scripts/censo-productores-de-aviso.mjs`
 * quedó de lápida. La conducción (A) lo posee; el instrumento es de E, entero.
 *
 * Rojo (exit 1): un tipo que EMITIÓ y hoy no tiene productor por NINGÚN método.
 * Eso es regresión, no deuda: `pedido_nuevo_vendedor` la tiene desde el 21-ago
 * (CREATE OR REPLACE se llevó su productor). No se apaga editando el censo: se
 * apaga cuando alguien reponga el productor. Deuda de construcción (D-673): un
 * tipo sin productor que NUNCA emitió — sale listado, no es rojo.
 */
