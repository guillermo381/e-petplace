...
/* ═══ ④ LA MEDICIÓN — LA HACE EL GUARD, NO ESTE GUION.

   ⏪ Acá vivía una sonda embebida que medía el solape en UN eje. Con la fila
   en DOS PISOS empezó a dar números sin sentido (`aire: -208` porque el chip
   ya no está al lado sino ABAJO: comparar sus X no significa nada) — o sea
   que **habría reportado un rojo falso justo cuando la pantalla quedó bien**.

   MUERE (Ley 37) en vez de arreglarse, y el porqué es la lección de esta
   tanda: **dos instrumentos que contestan la misma pregunta con distinta
   geometría van a divergir**, y el día que diverjan nadie va a saber cuál
   creer. La medición vive en UN lugar:

     node scripts/verify-colision-fila.mjs --email <cuenta> \
       --keychain epetplace-siembra-s97 --ancla "<texto de la fila>"

   Ese guard mide las DOS preguntas (desborde y colisión) en LOS DOS EJES y
   **sale ≠0** — este guion produce las imágenes; el veredicto lo da él. */
