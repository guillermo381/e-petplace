-- REVERSA de 20260814230000_s97a_voz_handshake.sql
-- ESCRITA ANTES DE APLICAR: **es la definición VIVA capturada de
-- `pg_get_functiondef` antes de empalmar**, o sea la reversa literal y no
-- una reconstrucción.
--
-- QUÉ DESHACE: quita la rama `autorizacion_mostrador_solicitada`.
-- 🔴 QUÉ NO DESHACE: los avisos ya enviados con la voz buena quedan como
--    están. Y revertir devuelve el aviso al genérico «tienes una novedad» —
--    que llega igual, pero **deja de decir que alguien está esperando**.
--    *La notificación no se rompe: se vuelve ignorable, que es peor de
--    detectar.*
