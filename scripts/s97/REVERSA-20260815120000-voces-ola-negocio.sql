-- REVERSA de 20260815120000_s97a_voces_ola_negocio.sql
-- ESCRITA ANTES DE APLICAR: definición VIVA de `_voz_notificacion` capturada
-- antes de empalmar. NO es una reconstrucción.
--
-- 🔴 QUÉ NO DESHACE: los tipos del catálogo QUEDAN (se DESACTIVAN aparte, no
--    se borran: hay intenciones que los referencian). Y revertir devuelve los
--    seis avisos al genérico «tenés una novedad» — **llegan igual, pero dejan
--    de decir qué pasó.** *La notificación no se rompe: se vuelve ignorable,
--    que es peor de detectar.*

BEGIN;

UPDATE cat_notificacion_tipos SET activo = false
 WHERE codigo IN ('pedido_nuevo_vendedor','naturaleza_venta_aprobada','solicitud_mostrador_expirada');
