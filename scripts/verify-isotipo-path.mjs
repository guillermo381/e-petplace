...
/* ── ☠️ ACÁ VIVÍAN LOS DOS BRAZOS DEL PAPEL, y murieron con su sujeto ──
   A los agregó al cablear el path v5 en `_shared/papel.ts`: uno comparaba la
   COPIA del papel contra esta fuente única, el otro exigía el encuadre por
   caja. **Los dos se retiran porque el papel ya no lleva un path** — la marca
   de agua de los seis documentos pasó a ser el LOGO COMPLETO como imagen
   (`D-1122`, rechazo del founder en el recorrido 5).

   ⚠️ **Y se retiran del todo, no se dejan en verde flojo:** un brazo que
   vigila una copia que ya no existe **da verde por ausencia** y se lee como
   «están sincronizados». *Un gate que aprueba lo que no puede encontrar es
   peor que uno que falta.*

   **Lo que este gate SIGUE midiendo es de `packages/ui`**, donde el path v5
   está vivo: la app lo usa en `Isotipo.tsx` con `dibujo="v5"`. */
...
console.log(
  fallos.length === 0
    ? `\nverify:isotipo-path — VERDE · 1 path · 0 clips · ${d.length} caracteres (la pieza de packages/ui)\n` +
