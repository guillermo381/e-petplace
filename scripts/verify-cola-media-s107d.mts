...
const ESTADIA = 'estadia-1';
const BASE = { tipo: 'foto' as const, mascotaIds: ['thor', 'zeus'], estadiaId: ESTADIA };
...
async function main() {
  ...
  // ── 1 · sobrevive al cierre ────────────────────────────────────────────
  {
    ...
    const tras = await pendientesDe(ESTADIA);
  ...
  // ── 2 · red caída no pierde nada ───────────────────────────────────────
  {
    ...
    const r = await procesarCola(motor, { estadiaId: ESTADIA });
    const [it] = await pendientesDe(ESTADIA);
  ...
  // ── 3 · 🔴 EL DISCRIMINADOR: no re-sube el byte ────────────────────────
  {
    ...
    const [tras1] = await pendientesDe(ESTADIA);
    ...
    await procesarCola(motor, { estadiaId: ESTADIA, ahora: luego });
    ...
    const pend = await pendientesDe(ESTADIA);
  ...
  // ── 4 · agotar reintentos: error visible, jamás descarte ───────────────
  {
    ...
    for (let i = 0; i < INTENTOS_AUTOMATICOS + 2; i += 1) {
      ahora += 60 * 60 * 1000;
      await procesarCola(motor, { estadiaId: ESTADIA, ahora });
    }
    const [it] = await pendientesDe(ESTADIA);
    ...
    const [revivido] = await pendientesDe(ESTADIA);
    ok(revivido?.estado === 'en_cola' && revivido.intentos === 0, '8 · el reintento a mano lo revive');

    await descartar(revivido!.id);
    ok((await pendientesDe(ESTADIA)).length === 0, '8b · descartar es explícito y funciona');
  }
  ...
  // ── 5 · el backoff se respeta ──────────────────────────────────────────
  {
    ...
    await procesarCola(motor, { estadiaId: ESTADIA });
    await procesarCola(motor, { estadiaId: ESTADIA }); // sin avanzar el reloj
  ...
  // ── 6 · sin motor: guarda y lo dice ────────────────────────────────────
  {
    ...
    const r = await procesarCola(null, { estadiaId: ESTADIA });
    const [it] = await pendientesDe(ESTADIA);
