...
do $seed$
declare
  -- ⇦ REEMPLAZAR por el uuid del user demo-vet creado por el founder:
  ...
  v_of_c  uuid := 'de680000-0000-4000-8000-00000000c001';  -- consulta_general
  v_of_v  uuid := 'de680000-0000-4000-8000-00000000c002';  -- vacunacion
  v_of_ul uuid := 'de680000-0000-4000-8000-00000000c003';  -- urgencia_local
  v_of_ud uuid := 'de680000-0000-4000-8000-00000000c004';  -- urgencia_domicilio
  v_esp1  uuid := 'de680000-0000-4000-8000-00000000e501';
  v_esp2  uuid := 'de680000-0000-4000-8000-00000000e502';
  v_esp3  uuid := 'de680000-0000-4000-8000-00000000e503';
  v_dia   int;
begin
  ...
  -- Ofertas del menú v1 (consulta + vacunación) + urgencia same-day.
  -- nombre_custom NULL: la vitrina dice la voz canónica del catálogo.
  insert into prestador_servicios
    (id, prestador_id, tipo_servicio, precio, duracion_minutos, activo, atiende_local, atiende_domicilio)
  values
    (v_of_c,  v_prest, 'consulta_general',   20, 30, true, true, false),
    (v_of_v,  v_prest, 'vacunacion',         15, 20, true, true, false),
    (v_of_ul, v_prest, 'urgencia_local',     30, 30, true, true, false),
    (v_of_ud, v_prest, 'urgencia_domicilio', 40, 45, true, true, true)
  on conflict (id) do nothing;

  -- Especialidades: dos del catálogo + una LIBRE ("Otra" = nombre_libre,
  -- jamás inserta al catálogo — la promoción es del admin).
  insert into prestador_especialidades (id, prestador_id, especialidad_id)
  select v_esp1, v_prest, ce.id from cat_especialidades_vet ce where ce.codigo = 'dermatologia'
  on conflict (id) do nothing;
  insert into prestador_especialidades (id, prestador_id, especialidad_id)
  select v_esp2, v_prest, ce.id from cat_especialidades_vet ce where ce.codigo = 'medicina_interna'
  on conflict (id) do nothing;
  insert into prestador_especialidades (id, prestador_id, nombre_libre)
    values (v_esp3, v_prest, 'Etología clínica')
    on conflict (id) do nothing;

  -- Franjas del titular: L-V 09:00-13:00 y 14:00-18:00, sáb 09:00-12:00
  -- (modo_horarios universal default: servicio_id NULL).
  for v_dia in 1..5 loop
    ...
    select v_prest, v_emp, null, v_dia, h.ini, h.fin, 30, 1, true
    from (values ('09:00'::time, '13:00'::time), ('14:00'::time, '18:00'::time)) as h(ini, fin)
    where not exists (
      select 1 from prestador_horarios ph
      where ph.prestador_id = v_prest and ph.empleado_id = v_emp
        and ph.dia_semana = v_dia and ph.hora_inicio = h.ini
    );
  end loop;
  insert into prestador_horarios
    (prestador_id, empleado_id, servicio_id, dia_semana, hora_inicio, hora_fin, duracion_slot_minutos, max_citas_por_slot, activo)
  select v_prest, v_emp, null, 6, '09:00', '12:00', 30, 1, true
  where not exists (
    select 1 from prestador_horarios ph
    where ph.prestador_id = v_prest and ph.empleado_id = v_emp
      and ph.dia_semana = 6 and ph.hora_inicio = '09:00'::time
  );

  raise notice 'seed_demo_vet_s68: OK — [DEMO S68] Clínica Aurora activa con 4 ofertas, 3 especialidades, 11 franjas';
end;
$seed$;