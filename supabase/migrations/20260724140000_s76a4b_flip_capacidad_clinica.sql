-- S76-A4b — EL FLIP §6.2: el gate clinico pasa a leer el CHIP (es_medico), no el
-- cargo generico 'profesional'. Cierra el agujero: una peluquera con toggle
-- Prestador y chip solo-grooming obtendria acceso clinico por el cargo. Censo A4a:
-- cero camino grooming/paseo/adiestramiento toca los helpers -> el flip no alcanza
-- oficios ajenos. Aplica DESPUES de A2 v2 (el candado antes que la llave).
-- 76(g): DDL sin backfill -> NO RIGE para la migracion.
--
-- Nace empleado_tiene_capacidad_clinica: is_admin() O titular (brazo NO opcional)
-- O empleado activo con chip en oferta es_medico=true (NO se exige ps.activo:
-- desactivar una oferta no le quita el expediente al veterinario).
-- Los 6 sitios clinicos cambian UNA llamada empleado_tiene_rol(_,['dueño',
-- 'profesional']) -> empleado_tiene_capacidad_clinica(_). obtener_contacto_reserva_cita
-- NO se toca (ventanilla, array con 'recepcion').
...
REVOKE EXECUTE ON FUNCTION public.empleado_tiene_capacidad_clinica(uuid) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.empleado_tiene_capacidad_clinica(uuid) TO authenticated;

-- ── flip: abrir_caso_clinico ──────────────────────────────────────────────
...
-- ── flip: completar_historia_clinica ──────────────────────────────────────────────
...
-- ── flip: registrar_vacuna_mostrador ──────────────────────────────────────────────
...
-- ── flip: user_acceso_clinico_a_mascota ──────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.user_acceso_clinico_a_mascota(p_mascota_id uuid)
 RETURNS boolean
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
    ...
    WHERE map.mascota_id = p_mascota_id
      ...
      AND map.cuenta_comercial_id IN (
        ...
        WHERE pe.user_id = v_user_id AND pe.activo = true
          AND empleado_tiene_capacidad_clinica(pe.prestador_id)
      )
...
-- ── flip: user_puede_escribir_clinico ──────────────────────────────────────────────
...
AS $function$
  SELECT
    ...
    AND (
      is_admin()
      OR empleado_tiene_capacidad_clinica(p_prestador_id)
    );
$function$
