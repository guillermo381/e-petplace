-- ══════════════════════════════════════════════════════════════════════════
-- S105-A · EL REVERSO MUEVE EL SUJETO EN **LOS DOS RIELES** — por trigger
--
-- LA DIVERGENCIA VIVA, medida: **dos compras diciendo `pagada` con la plata ya
-- devuelta.**
--   · `9cfced78` · $94,50 · **deuna** · intento `reversado`
--   · `8eb62f9e` · $24,90 · **nuvei** · intento `reversado`
--
-- 🔴 POR QUÉ TRIGGER Y NO UNA LLAMADA POR RIEL — y es la lección del día:
-- `registrar_reverso_deuna` existe (pista D) y **no llama a
-- `mover_sujeto_por_reverso`**. No es un olvido: *cablear la llamada dentro de
-- cada registrador es cómo el segundo riel se olvida.* Nuvei tenía su cable y
-- DeUna nació sin él, con las dos piezas construidas y correctas.
--
-- **La regla real no es «cuando el actuador de Nuvei reconoce un reverso»:
-- es «cuando un intento entra en estado terminal».** Eso es una transición de
-- la tabla, no un evento de un proveedor ⇒ **vive en la tabla**. Un riel
-- futuro hereda el comportamiento sin acordarse de pedirlo.
--
-- Y `mover_sujeto_por_reverso` **ya era agnóstica de proveedor**: mira el
-- estado del intento y el sujeto, nunca quién lo reversó. No hubo que tocarla.
--
-- ⚠️ SE QUITA LA LLAMADA EXPLÍCITA DEL ACTUADOR — *dos caminos que hacen lo
-- mismo es cómo uno de los dos queda viejo.* El actuador ahora LEE el
-- resultado; el trigger es el que ordena.
--
-- ① EL TEXTO DEL PRESTADOR, ENMENDADO POR FIRMA DEL FOUNDER: «no tienes que
-- hacer nada» → «no se te cobrará nada por esta cita». *Su preocupación real
-- no es si hay un trámite — es si esa cita le suma o le resta plata.*
--
-- 76(g) — VEDA: 🔴 **RIGE.** Esta migración **mueve dos compras vivas** con su
-- pedido y su reserva de inventario. Ventana abierta y cerrada acá adentro; el
-- alcance está acotado por id y verificado antes y después.
-- REVERSA escrita ANTES: `docs/relevamientos/S105-A-REVERSA-20260826060000.sql`
-- ══════════════════════════════════════════════════════════════════════════
...
/* ② EL TRIGGER — la regla vive en la tabla, no en cada riel. */
...
AS $$
BEGIN
  /* Sólo en la ENTRADA al estado terminal: un UPDATE que toca otra columna de
     un intento ya reversado no vuelve a cancelar nada. */
  IF NEW.estado IN ('reversado','reverso_fallido')
     AND COALESCE(OLD.estado,'') NOT IN ('reversado','reverso_fallido') THEN
    /* 🔴 SE ATRAPA A PROPÓSITO. El registro del reverso **ya ocurrió** —la
       plata volvió del lado del proveedor—; si mover el sujeto falla, dejar
       caer la excepción revertiría también el registro y quedaríamos sin
       ninguna de las dos cosas. *El fallo se escribe con su nombre para que
       una persona lo vea, y el hecho del reverso se conserva.* */
    BEGIN
...
CREATE TRIGGER trg_pagos_intentos_reverso_mueve_sujeto
  AFTER UPDATE OF estado ON public.pagos_intentos
  FOR EACH ROW EXECUTE FUNCTION public._trg_reverso_mueve_sujeto();


/* ③ LAS DOS COMPRAS VIVAS — 76(g) RIGE ACÁ.
   El trigger no las alcanza: ya están en estado terminal, no hay UPDATE que
   lo dispare. Se mueven por la MISMA función, acotadas por id. */
DO $viva$
DECLARE r jsonb; v_id uuid;
BEGIN
  FOREACH v_id IN ARRAY ARRAY[
    (SELECT i.id FROM pagos_intentos i WHERE i.compra_id::text LIKE '9cfced78%' AND i.estado='reversado' LIMIT 1),
    (SELECT i.id FROM pagos_intentos i WHERE i.compra_id::text LIKE '8eb62f9e%' AND i.estado='reversado' LIMIT 1)
  ] LOOP
    IF v_id IS NULL THEN CONTINUE; END IF;
    r := mover_sujeto_por_reverso(v_id, 'reverso confirmado por el proveedor (S105)');
    RAISE NOTICE 'compra viva movida · intento=% · %', left(v_id::text,8), r::text;
  END LOOP;
END $viva$;
...
-- ══════════════════════════════════════════════════════════════════════════
-- CINTURÓN — 🔴 verifica lo que 76(g) obliga: qué quedó DE VERDAD en las filas
-- vivas, y que el trigger dispara solo.
-- ══════════════════════════════════════════════════════════════════════════
DO $cint$
DECLARE
  v_pagadas int; v_int uuid; v_prev text; v_compra uuid; v_est_c text; v_est_p text;
  v_ped uuid; v_res_id uuid; v_res text; v_res_cer timestamptz; v_despues text;
  v_voz jsonb;
BEGIN
  -- ① LAS DOS COMPRAS: ninguna puede seguir `pagada` con su intento reversado
  SELECT count(*) INTO v_pagadas FROM compras c
    JOIN pagos_intentos i ON i.compra_id = c.id
   WHERE i.estado IN ('reversado','reverso_fallido') AND c.estado = 'pagada';
  IF v_pagadas > 0 THEN
    RAISE EXCEPTION 'CINTURÓN: quedan % compras PAGADAS con la plata devuelta', v_pagadas;
  END IF;

  -- ② EL TRIGGER DISPARA SOLO — camino ejercido y deshecho (`L-406`)
  SELECT i.id, i.estado, i.compra_id INTO v_int, v_prev, v_compra
    FROM pagos_intentos i JOIN compras c ON c.id=i.compra_id
   WHERE i.estado='aprobado' AND c.estado='pagada' LIMIT 1;
  ...
  IF v_int IS NOT NULL THEN
    ...
    SELECT r.id, r.estado, r.cerrada_en INTO v_res_id, v_res, v_res_cer
      FROM inventario_reservas r WHERE r.pedido_id=v_ped LIMIT 1;

    UPDATE pagos_intentos SET estado='reversado' WHERE id=v_int;   -- ← dispara
    SELECT estado INTO v_despues FROM compras WHERE id=v_compra;

    -- deshacer ANTES de decidir
    UPDATE pagos_intentos SET estado=v_prev WHERE id=v_int;
    ...
    IF v_res_id IS NOT NULL THEN
      UPDATE inventario_reservas SET estado=v_res, cerrada_en=v_res_cer WHERE id=v_res_id;
    END IF;
    DELETE FROM notificacion_intencion WHERE clave_dedup LIKE 'reverso:%'
       AND created_at > now() - interval '1 minute';

    IF v_despues <> 'cancelada' THEN
      RAISE EXCEPTION 'CINTURÓN: el trigger NO movió el sujeto (compra quedó %)', v_despues;
    END IF;
  END IF;

  -- ③ EL TEXTO ENMENDADO
  v_voz := _voz_notificacion('pago_reversado',(SELECT id FROM auth.users LIMIT 1),NULL,'{"sujeto":"cita"}'::jsonb);
  IF v_voz->>'mensaje' NOT ILIKE '%no se te cobrará nada%' THEN   -- ILIKE: el texto abre con mayúscula
    RAISE EXCEPTION 'CINTURÓN: el texto del prestador no lleva la enmienda — %', v_voz->>'mensaje';
  END IF;

  RAISE NOTICE 'CINTURÓN VERDE · compras pagadas con plata devuelta=% · trigger dispara solo=% · texto enmendado OK',
    v_pagadas, (v_int IS NOT NULL);
END $cint$;

-- ④ EL AVISO SE ENCIENDE — firma del founder sobre los dos textos.
UPDATE public.cat_notificacion_tipos SET en_sombra = false WHERE codigo = 'pago_reversado';
