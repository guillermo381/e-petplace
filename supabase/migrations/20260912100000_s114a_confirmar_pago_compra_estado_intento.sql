-- S114-A · PAGO MIXTO · el brazo mixto de confirmar_pago_compra miraba el estado
-- equivocado del intento — bug de una letra, medido en vivo (incidente founder 9-sep).
--
-- El edge (pagos-cobro) escribe el intento aprobado como `pendiente`
-- (`estado: aprobado ? 'pendiente' : 'rechazado'`), y `'iniciado'` NO existe en la
-- tabla (estados vivos: pendiente/aprobado/rechazado/reversado). El brazo mixto
-- contaba `estado IN ('iniciado','aprobado')` ⇒ 0 ⇒ disparaba
-- `intento_no_unico_en_compra_mixta` RUIDOSO sobre premisa falsa, el webhook quedaba
-- en `recibido` y la compra mixta nunca se confirmaba (saldo reservado sin consumir,
-- riel ya aprobado). El chequeo ② de la firma funcionó — miraba el estado equivocado.
--
-- CURA: `estado IN ('pendiente','aprobado')` en el count/consistencia y en el UPDATE.
-- Sólo el brazo mixto; el camino sin saldo (confirmar_pago_pedido) no se toca.
--
-- 76(g) NO RIGE (DB, sin anclas de OTA). Reversa ANTES en
-- docs/relevamientos/2026-09-09-s114a-REVERSA-20260912100000-confirmar-estado-intento.sql
...
BEGIN
  ...
  IF v_saldo > 0 THEN
    ...
    SELECT count(*), max(monto) INTO v_int_ct, v_int_monto
      FROM pagos_intentos
     WHERE compra_id = p_compra_id AND estado IN ('pendiente','aprobado');
    IF v_int_ct <> 1 THEN
...
-- ── CINTURÓN · el brazo mixto ya mira 'pendiente' y no 'iniciado' ────────────
DO $cinturon$
DECLARE v_def text;
BEGIN
  v_def := pg_get_functiondef('public.confirmar_pago_compra(uuid,text,text,text,jsonb,text,text,numeric,text,text,text,text)'::regprocedure);
  IF v_def ILIKE '%estado IN (''iniciado'',''aprobado'')%' THEN
    RAISE EXCEPTION 'CINTURÓN: quedó una ocurrencia del filtro viejo (iniciado)';
  END IF;
  IF (length(v_def) - length(replace(v_def, 'estado IN (''pendiente'',''aprobado'')', ''))) / length('estado IN (''pendiente'',''aprobado'')') <> 2 THEN
    RAISE EXCEPTION 'CINTURÓN: se esperaban 2 ocurrencias del filtro nuevo (pendiente,aprobado)';
  END IF;
  RAISE NOTICE 'CINTURÓN VERDE · brazo mixto mira (pendiente,aprobado) · 2 ocurrencias · cero (iniciado)';
END $cinturon$;
