-- S114-A · el estado de aviso deja de llamarse `entregada` — porque no lo sabe.
-- Firma del founder (10-sep): `entregada` = «FCM aceptó» (HTTP 200), NO «el aparato
-- recibió». Prueba viva: 3 avisos + 1 de prueba «entregados» que el founder nunca
-- recibió (token fantasma: FCM 200, aparato sin la registración). FCM v1 NO da
-- receipt por-mensaje al emisor; un ACK de la app sería parcial (foreground sí,
-- background no). No se puede confirmar recepción barato ⇒ el estado se renombra a
-- lo que SÍ sabe: `aceptada_transporte`. La clase del arco: un nombre que promete
-- más de lo que la medición sostiene.
--
-- QUIRÚRGICO: SÓLO notificacion_intencion.estado. El `entregada` de guardería (la
-- mascota entregada) y de pedidos (compra entregada) son OTROS estados, de OTRAS
-- tablas — NO se tocan.
--
-- Ventana segura: CHECK admite AMBOS y las funciones cuentan AMBOS mientras los 3
-- edges se redepliegan al valor nuevo. Tighten (quitar `entregada`) = seguimiento.
...
ALTER TABLE notificacion_intencion ADD CONSTRAINT notificacion_intencion_estado_check
  CHECK (estado = ANY (ARRAY['nacida','encolada','entregada','aceptada_transporte','leida','descartada','fallida','diferida']));

UPDATE notificacion_intencion SET estado='aceptada_transporte' WHERE estado='entregada';
  ...
  IF v_motivo IS NULL THEN
    SELECT count(*) INTO v_cuantas
     ...
     WHERE i.destinatario_user_id = p_destinatario_user_id
       ...
       AND i.estado IN ('nacida','encolada','entregada','aceptada_transporte','leida')
  ...
  LOOP
    ...
    IF v_motivo IS NULL THEN
      SELECT count(*) INTO v_entregadas
        FROM public.notificacion_intencion i
       WHERE i.estado IN ('entregada','aceptada_transporte','leida')
...
DO $cinturon$
DECLARE v_viejas int; v_check text;
BEGIN
  SELECT count(*) INTO v_viejas FROM notificacion_intencion WHERE estado='entregada';
  IF v_viejas <> 0 THEN RAISE EXCEPTION 'CINTURON: quedan % en entregada', v_viejas; END IF;
  v_check := pg_get_constraintdef((SELECT oid FROM pg_constraint WHERE conname='notificacion_intencion_estado_check'));
  IF v_check NOT LIKE '%aceptada_transporte%' THEN RAISE EXCEPTION 'CINTURON: CHECK sin aceptada_transporte'; END IF;
  RAISE NOTICE 'CINTURON VERDE - 0 en entregada, CHECK admite aceptada_transporte, funciones cuentan ambos';
END $cinturon$;
