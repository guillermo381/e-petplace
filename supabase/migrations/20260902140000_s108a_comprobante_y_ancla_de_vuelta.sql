-- ═══════════════════════════════════════════════════════════════════════════
-- S108-A · M7 · EL COMPROBANTE DE LOS DOS SUJETOS, Y EL ANCLA VUELVE AL INTENTO
--
-- 76(g) VEDA: **NO RIGE.** Reemplazo de una función. Cero backfill.
-- REVERSA: `docs/relevamientos/2026-09-02-s108a-REVERSA-M7.sql`.
-- 🤝 CRUCE: el bloque del comprobante lo escribió y lo EJERCIÓ **S108-B**; llegó
--    como texto SQL completo y autocontenido y **se pegó literal**. A le agregó
--    únicamente su BEGIN/EXCEPTION, declarado adentro. Depende de
--    `guarderia_suscripcion_desglose` (B, `20260902100000`) ⇒ este número va
--    después a propósito.
-- ═══════════════════════════════════════════════════════════════════════════
...
DECLARE v_fam uuid; v_prest uuid; v_user uuid; v_bono uuid; v_int uuid;
        v_evt uuid; v_r jsonb; v_total numeric; v_n int;
BEGIN
  ...
  IF v_total IS NULL THEN RAISE EXCEPTION 'cinturon M7: sin desglose congelado'; END IF;
  ...
  VALUES (v_bono,'nuvei',v_total,'USD','tokenizacion','pendiente',
          'cinturon:m7:'||v_bono::text,'{}'::jsonb, v_user,'sesion')
  ...
  VALUES ('nuvei', jsonb_build_object('transaction',
            jsonb_build_object('dev_reference', v_bono::text, 'status','1',
                               'amount', v_total::text,'id','tx-m7','status_detail','3')),
          'recibido','', true, 'sandbox', 'SERVER')
  RETURNING id INTO v_evt;

  v_r := public.aplicar_evento_de_pago(v_evt);
  IF COALESCE((v_r->>'aplicado')::boolean,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'cinturon M7: no se acredito: %', v_r::text;
  END IF;

  -- (a) 🔴 EL COMPROBANTE SALIÓ, y con la clave que B decidió
  SELECT count(*) INTO v_n FROM notificacion_intencion
   WHERE clave_dedup = 'comprobante:' || v_bono::text;
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'cinturon M7: el comprobante del paquete no salio (n=%)', v_n;
  END IF;

  -- (b) el ancla vuelve al intento: cableado (la mensualidad no se ejerce aca,
  --     exige generar un mes entero; se verifica que la linea EXISTA)
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='aplicar_evento_de_pago'
     AND pg_get_functiondef(p.oid) LIKE '%SET guarderia_suscripcion_periodo = (v_acto->>''periodo_desde'')::date%';
  IF v_n <> 1 THEN RAISE EXCEPTION 'cinturon M7: el write-back del ancla no quedo'; END IF;

  RAISE NOTICE 'cinturon M7: 2/2 OK (comprobante del paquete emitido con su clave · write-back del ancla cableado)';
  RAISE NOTICE 'cinturon M7: ⚠️ el comprobante de la MENSUALIDAD no se ejercio — exige generar un mes completo con cupo. Declarado.';
  RAISE EXCEPTION 'FIXTURE_ROLLBACK_OK';
EXCEPTION WHEN OTHERS THEN
  IF SQLERRM = 'FIXTURE_ROLLBACK_OK' THEN
    RAISE NOTICE 'cinturon M7: fixture deshecho por subtransaccion, residuo 0';
  ELSE RAISE;
  END IF;
END $c$;
