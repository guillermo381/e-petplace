-- ═══════════════════════════════════════════════════════════════════════════
-- S103-A · EL ACTUADOR LEE LA VERDAD VERIFICADA, NO LA SEÑAL
--
-- 🔴 EL DEFECTO, hallado por S103-D revisando el cuerpo vivo — y era de
--    CONTRATO, no de traducción. El buzón guarda:
--
--        payload: { webhook: <lo que llegó>, info: <lo que confirmamos> }
--
--    y el actuador leía `payload->>'status'` en la RAÍZ. ⇒ `v_refcorta` salía
--    NULL, el SELECT no matcheaba, y devolvía `sin_referencia_corta`. Y aunque
--    el sujeto se hubiera resuelto, `_pago_aprobado` tampoco habría visto
--    `status` ⇒ **nunca aprobaba.** Dos fallas independientes, cualquiera
--    alcanzaba para que el lunes se cobrara y el pedido no avanzara.
--
--    *El diff conceptual decía RAÍZ y era cierto cuando se escribió; el buzón
--    se enriqueció después. Nadie tradujo mal: el contrato cambió abajo.*
--
-- 🔴 PERO LA ANIDACIÓN ES EL SÍNTOMA. EL PROBLEMA ES LA FUENTE DE VERDAD.
--    `LETRA_DEUNA` §7: **el webhook es SEÑAL, no verdad; sólo la respuesta
--    verificada por `payment/info` alimenta al actuador.** El buzón guarda las
--    dos JUSTAMENTE por eso.
--    ⇒ Un actuador que leyera del `webhook` estaría **confirmando pagos con la
--    señal**, y el monto que compara contra el desglose congelado vendría de un
--    mensaje que cualquiera con el secreto puede fabricar.
--
-- ⚖️ POR QUÉ **FAIL-CLOSED** Y NO EL `COALESCE(payload->'info', payload)` QUE
--    D OFRECIÓ — y la razón la dio él mismo al ofrecerlo: *«si algún día el
--    buzón cambia otra vez, el fallback la esconde».*
--    **Un puente que tolera las dos formas convierte el próximo cambio de
--    contrato en otro defecto silencioso** — exactamente la clase que esta
--    migración viene a cerrar. Y hay una razón de letra encima de la de
--    método: **si `info` no está, la consulta activa NO ocurrió, y §7 dice que
--    entonces NO SE CONFIRMA.** *No es tolerancia lo que corresponde: es
--    negarse.* Nace el motivo `sin_consulta_verificada`, con nombre propio.
--
--    *(Consistencia, declarada: `_evento_autenticado` ya exige
--     `detalle ILIKE '%verificado=si%'` para DeUna, que el buzón sólo escribe
--     DESPUÉS de que `payment/info` confirmó. Un evento que pasa la puerta y
--     no trae `info` es una incoherencia entre dos piezas nuestras — y eso se
--     NOMBRA, no se absorbe.)*
--
-- 📌 76(g) — LA VEDA: **NO RIGE.** `CREATE OR REPLACE` de un cuerpo. Cero DDL
--    sobre datos, cero backfill, cero anclas a filas vivas.
--
-- ── REVERSA ────────────────────────────────────────────────────────────────
-- Volver a `v_e.payload` plano en la rama DeUna.
-- ⚠️ QUÉ NO DESHACE: nada de datos. Pero **reintroduce el defecto**: el riel
-- DeUna vuelve a no reconocer un solo pago.
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

-- Sólo cambia la rama DeUna: de dónde SALEN los cinco campos. Nuvei intacto.
...
DECLARE
  ...
  v_es_cita boolean; v_intento uuid; v_refcorta text; v_tocadas int; v_src jsonb;
BEGIN
...
BEGIN
  ...
  -- (a) Lee de `info` y se NIEGA sin ella.
  IF position($lit$v_e.payload->'info'$lit$ IN v_def) = 0 THEN
    RAISE EXCEPTION 'ABORTA: el actuador no lee la respuesta verificada';
  END IF;
  IF position('sin_consulta_verificada' IN v_def) = 0 THEN
    RAISE EXCEPTION 'ABORTA: falta el fail-closed de §7';
  END IF;

  -- (b) NO quedó ningún COALESCE que tolere la forma vieja. *Ese es el punto
  --     de esta migración: la tolerancia era el defecto.*
  IF position($lit$COALESCE(v_e.payload->'info', v_e.payload)$lit$ IN v_def) > 0 THEN
    RAISE EXCEPTION 'ABORTA: quedo el puente tolerante que esconde el proximo cambio';
  END IF;

  -- (c) Nuvei sigue vivo y sin literal de proveedor.
  IF position('dev_reference' IN v_def) = 0 THEN
    RAISE EXCEPTION 'ABORTA: se perdio la rama de Nuvei';
  END IF;
  IF position($lit$p_proveedor => 'nuvei'$lit$ IN v_def) > 0 THEN
    RAISE EXCEPTION 'ABORTA: volvio el literal nuvei';
  END IF;

  -- (d) `no_verificado` es vocabulario legal de `webhook_events.resultado`
  --     (lo agregó N2). *Escribir un estado que el CHECK rechaza haría que el
  --     fail-closed ABORTE la transacción entera en vez de negarse limpio.*
  IF (SELECT count(*) FROM pg_constraint
       WHERE conname='webhook_events_resultado_check'
         AND position('no_verificado' IN pg_get_constraintdef(oid)) > 0) <> 1 THEN
    RAISE EXCEPTION 'ABORTA: `no_verificado` no es un resultado legal';
  END IF;

  RAISE NOTICE 'CINTURON OK: lee `info` · se niega sin ella · sin puente tolerante · Nuvei vivo · `no_verificado` legal';
END $cinturon$;
