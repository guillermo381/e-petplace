-- ════════════════════════════════════════════════════════════════════════════
-- S113-A · la ruta del papel apuntaba a una pantalla que no existe (rojo de E)
--
-- Censo contra el árbol, las SIETE: mascota ✓ · cita ✓ · pedido ✓ · recuerdo ✓
-- · producto ✓ · prestador ✓ · **papel 🔴**. La única rota era la que agregué
-- en la fase 3.
--
-- *Un resultado con ruta muerta se ve perfecto: el problema aparece recién en
-- el dedo de alguien.* Y no hay gate del tren normal que lo vea — para el motor
-- una ruta es un string.
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  pa as (
    ...
    select 'papel' as tipo, pf.id::text,
           ...
           /* 🔴 LA RUTA QUE YO INVENTÉ NO EXISTÍA. `/hogar/mascota/<id>/papeles`
              no está en ninguna rama —medido contra el árbol: 6 de las 7 rutas
              que emite esta RPC existen, y la única rota era la que agregué—.
              *Un resultado con ruta muerta se ve perfecto y el toque no lleva a
              nada; ningún gate del tren normal lo ve, porque para el motor es
              un string.* Lo cazó el gate de rutas de E.
              Apunta al PERFIL, que existe y es donde la bóveda va a vivir.
              Cuando C tenga su pantalla propia, es esta línea. */
           '/hogar/mascota/' || pf.mascota_id as ruta,
           ts_rank(to_tsvector('spanish',
