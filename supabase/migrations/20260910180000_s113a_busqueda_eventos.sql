-- ════════════════════════════════════════════════════════════════════════════
-- S113-A · rojo ⑨ de E/D — el índice del expediente veía 8 de 520 eventos
--
-- Leía tres claves genéricas (`texto`, `nota`, `mensaje`) y **sólo `fin_vida`
-- usa `nota`**: cada tipo guarda lo suyo en su propia clave. *Una lista de
-- claves escrita a mano mide la convención del día que se escribió, y el tipo
-- número doce no la conoce.*
--
-- ⚠️ ESTA MIGRACIÓN SE REESCRIBIÓ UNA VEZ, y la razón vale más que el cambio:
-- la primera versión salió de un cuerpo que yo había guardado en `/tmp` ANTES
-- de aplicar las curas ① y ②, y al aplicarla **las pisó las dos** —«alimento»
-- volvió a 0 y «clinica» a sólo citas—. *Un cuerpo guardado en un archivo
-- envejece; el objeto no.* Es la misma trampa que esta serie de migraciones
-- viene evitando al sacar el cuerpo con `pg_get_functiondef`, cobrada por
-- sacarlo una vez y usarlo dos.
-- ════════════════════════════════════════════════════════════════════════════
