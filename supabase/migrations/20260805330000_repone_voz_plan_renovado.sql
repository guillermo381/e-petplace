-- S88-A · REPONE la voz de `plan_renovado` — perdida al reescribir la función
-- sobre un snapshot viejo, y cazada POR EL PAR.
--
-- 76(g) — VEDA: NO RIGE. `CREATE OR REPLACE` de una función.
--
-- ⚠️ LA LECCIÓN, en el sitio donde se paga:
--   el lote de voces reescribió `cerrar_y_renovar_planes` partiendo de un
--   `pg_get_functiondef` capturado MINUTOS ANTES — antes de que la migración
--   de `plan_renovado` existiera. El `CREATE OR REPLACE` no falló: **hizo
--   exactamente lo que se le pidió**, y borró una voz que ya estaba en
--   producción.
--   **El par lo cazó en el acto**: la sombra devolvió asunto NULL para el ÚNICO
--   tipo fuera de sombra — el que sale solo el 13-ago.
--
-- > **Un `CREATE OR REPLACE` se arma leyendo el objeto VIVO, jamás una copia
-- > guardada.** *Y lo que lo hace peligroso es que no hay conflicto que avise:
-- > sobrescribir una función es siempre «exitoso».*
...
DO $belt$
DECLARE v_def text := pg_get_functiondef('public.cerrar_y_renovar_planes()'::regprocedure);
BEGIN
  IF v_def NOT LIKE '%_voz_notificacion(''plan_renovado''%' THEN
    RAISE EXCEPTION 'CINTURON: plan_renovado sigue sin voz';
  END IF;
  IF v_def NOT LIKE '%_voz_notificacion(''plan_renovacion_proxima''%'
     OR v_def NOT LIKE '%_voz_notificacion(''plan_vencido_reembolso''%' THEN
    RAISE EXCEPTION 'CINTURON: se cayó alguna de las voces del lote';
  END IF;
  RAISE NOTICE 'CINTURON VERDE: las TRES voces de esta función, presentes.';
END
$belt$;
