-- ═══════════════════════════════════════════════════════════════════════════
-- S109-A · EL `ELSE` CIEGO DEL REVERSO, Y LA PUERTA DEL PLAN DE PASEO
--
-- 76(g) VEDA: **NO RIGE.** Dos reemplazos. Cero backfill.
-- REVERSA: `docs/relevamientos/2026-09-06-s109a-REVERSA-M24.sql`.
--
-- ═══ LOS DOS SALIERON DEL CENSO QUE EL FOUNDER MANDÓ CORRER ════════════════
-- 🔴 ① **EL `ELSE` CIEGO.** `mover_sujeto_por_reverso` cerraba su cuerpo con un
--    `ELSE` que significaba `recurrencia`. Con DOS sujetos eso era un XOR
--    correcto; **con SIETE pasó a significar «todo lo que no es ninguno de los
--    anteriores»** ⇒ un sujeto nuevo sin rama **cancelaba una recurrencia de
--    despensa que no era suya**, con `ok:true`. *Compila, corre, y devuelve un
--    resultado plausible sobre el objeto equivocado.*
--    Es el mismo defecto que S108-B encontró en su edge, un piso más abajo y
--    con más sujetos. Ahora la rama es explícita y **un sujeto sin rama grita**.
--
-- 🔴 ② **`confirmar_pago_plan_paseo` NO TENÍA LLAMADOR** — `L-318`, motor sin
--    puerta, censado sobre el cuerpo sin comentarios: cero funciones lo nombran.
--    *La pieza estaba bien construida, probada, y desconectada del único lugar
--    donde su resultado importa.*
--
--    ⚠️ Y cablearla obligó a una distinción que no existía: **el primer cobro
--    CONFIRMA, no renueva.** `renovar_plan_cobrado` es para el mes 2 en
--    adelante; llamarla sobre un plan sin estrenar movería un período que no
--    existe. Se distingue por `estado_pago = 'pendiente'`, que es el HECHO —
--    no por el período, que es su consecuencia.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ══ RECURRENCIA ══
  /* ✏️ ERA UN `ELSE` CIEGO, y con SIETE sujetos eso dejó de ser un XOR.
     🔴 «Todo lo que no es ninguno de los anteriores» significaba `recurrencia`,
     así que un sujeto nuevo que no tuviera rama **cancelaba una recurrencia de
     despensa que no era suya** — y no da error: compila, corre, y devuelve
     `ok:true` sobre el objeto equivocado. *Es el mismo defecto que S108-B
     encontró en su edge, un piso más abajo y con más sujetos.* */
  ELSIF v_sujeto = 'recurrencia' THEN
    UPDATE pedidos_recurrencias SET estado = 'cancelada' WHERE id = v_id AND estado <> 'cancelada';

  ELSE
    /* Un sujeto que el resolvedor nombra y este cuerpo no sabe mover **grita**.
       Antes se lo llevaba puesto la rama de recurrencia. */
    RETURN jsonb_build_object('ok', false, 'codigo','sujeto_sin_rama_de_reverso',
                              'sujeto', v_sujeto, 'sujeto_id', v_id);
  END IF;
...
-- ═══ CINTURÓN ═════════════════════════════════════════════════════════════
DO $c$
DECLARE v_n int; v_r jsonb; v_int uuid; v_user uuid; v_ped uuid; v_estado text;
BEGIN
  -- (a) 🔴 EL DISCRIMINADOR DEL ELSE: se FABRICA un intento cuyo sujeto el
  --     cuerpo no sabe mover, y se exige que NO toque ninguna recurrencia.
  SELECT id, estado INTO v_ped, v_estado FROM pedidos_recurrencias ORDER BY created_at LIMIT 1;

  SELECT b.user_id INTO v_user FROM bonos b ORDER BY b.created_at LIMIT 1;
  ...
  SELECT pc.id,'nuvei',10,'USD','tokenizacion','aprobado','cinturon:m24:'||pc.id::text,
         '{}'::jsonb, v_user,'sesion'
    FROM programas_contratados pc ORDER BY pc.created_at LIMIT 1
  RETURNING id INTO v_int;

  /* Se le saca la rama al programa a la fuerza: se apunta el intento a un
     sujeto que el resolvedor nombra. Como `programa` SÍ tiene rama, el brazo
     real es el (b): que el ELSE ya no exista como cajón. */
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='mover_sujeto_por_reverso'
     AND regexp_replace(regexp_replace(p.prosrc,'/\*.*?\*/','','gs'),'--[^\n]*','','g')
         LIKE '%sujeto_sin_rama_de_reverso%';
  IF v_n <> 1 THEN RAISE EXCEPTION 'cinturon: el ELSE sigue siendo un cajon ciego'; END IF;

  -- (b) la rama de recurrencia quedó EXPLÍCITA (no se perdió al cerrar el else)
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='mover_sujeto_por_reverso'
     AND regexp_replace(regexp_replace(p.prosrc,'/\*.*?\*/','','gs'),'--[^\n]*','','g')
         LIKE '%ELSIF v_sujeto = ''recurrencia''%';
  IF v_n <> 1 THEN RAISE EXCEPTION 'cinturon: al cerrar el else se perdio la recurrencia'; END IF;

  -- (c) 🔴 EL REVERSO DEL PROGRAMA SIGUE FUNCIONANDO (no se rompió al tocar el else)
  UPDATE pagos_intentos SET estado='reversado' WHERE id=v_int;
  IF NOT EXISTS (SELECT 1 FROM programas_contratados pc
                  JOIN pagos_intentos i ON i.programa_contratado_id = pc.id
                 WHERE i.id=v_int AND pc.estado_pago='reembolsado') THEN
    RAISE EXCEPTION 'cinturon: cerrar el else rompio el reverso del programa';
  END IF;

  -- (d) y la recurrencia de despensa NO se tocó
  IF v_ped IS NOT NULL THEN
    IF (SELECT estado FROM pedidos_recurrencias WHERE id=v_ped) IS DISTINCT FROM v_estado THEN
      RAISE EXCEPTION 'cinturon: el reverso de un programa toco una RECURRENCIA ajena';
    END IF;
  ELSE
    RAISE NOTICE 'cinturon M24: ⚠️ brazo (d) NO EJERCIDO — no hay recurrencia con que discriminar';
  END IF;

  -- (e) 🔴 LA PUERTA: el actuador ya nombra a confirmar_pago_plan_paseo
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='aplicar_evento_de_pago'
     AND regexp_replace(regexp_replace(p.prosrc,'/\*.*?\*/','','gs'),'--[^\n]*','','g')
         LIKE '%confirmar_pago_plan_paseo%';
  IF v_n <> 1 THEN RAISE EXCEPTION 'cinturon: confirmar_pago_plan_paseo sigue SIN puerta'; END IF;

  RAISE NOTICE 'cinturon M24: 5/5 OK (el else grita · la recurrencia quedo explicita · el reverso del programa intacto · cero dano colateral · la puerta del plan existe)';
  RAISE EXCEPTION 'FIXTURE_ROLLBACK_OK';
EXCEPTION WHEN OTHERS THEN
  IF SQLERRM = 'FIXTURE_ROLLBACK_OK' THEN
    RAISE NOTICE 'cinturon M24: fixture deshecho por subtransaccion, residuo 0';
  ELSE RAISE;
  END IF;
END $c$;
