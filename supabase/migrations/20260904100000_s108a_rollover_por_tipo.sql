-- ═══════════════════════════════════════════════════════════════════════════
-- S108-A-6 · ① EL ROLLOVER RESUELVE POR TIPO — y la función deja de mentir
--
-- 76(g) VEDA: **NO RIGE.** Renombre + reemplazo de funciones. Cero backfill.
-- REVERSA: `docs/relevamientos/2026-09-04-s108a-REVERSA-M14.sql`.
-- L-119/L-120: el renombre es DROP + CREATE, con sus callers censados ANTES —
--   **dos en DB** (`aplicar_evento_de_pago` y una lápida en
--   `comprar_paquete_guarderia`), **cero en el repo**. Los dos se mueven acá.
--
-- ═══ POR QUÉ ESTE PASO VA PRIMERO ══════════════════════════════════════════
-- 🔴 Orden del founder, y su razón es la correcta: **si el bono de paseo cambia
--    de estado inicial ANTES de que su rollover tenga dónde correr, el rollover
--    se pierde EN SILENCIO** — y eso es saldo que la familia pagó y no recibe,
--    sin error y sin síntoma, porque el bono queda pagado igual.
--
-- ═══ LAS DOS REGLAS DIFIEREN DE VERDAD, no es un filtro más ════════════════
--   · guardería → `(familia, prestador)`
--   · paseo     → `(familia, prestador, PRESTADOR_SERVICIO)`
-- *El paquete de paseo es de una oferta concreta; el de guardería es del lugar.*
-- Por eso el rollover se resuelve **POR TIPO**, y **un tipo sin rama declarada
-- REVIENTA**: el día que `bonos_tipo_valido` admita un tercero, esta función
-- falla ruidosamente en vez de confirmarlo sin rollover.
--
-- ═══ Y EL NOMBRE SE ARREGLA AHORA ══════════════════════════════════════════
-- ☠️ `confirmar_pago_paquete_guarderia` → **`confirmar_pago_bono`**. El actuador
--    la llamaba para CUALQUIER bono: una función `…_guarderia` confirmando
--    paquetes de paseo es cómo el próximo la lee como si fuera de un oficio.
-- ═══════════════════════════════════════════════════════════════════════════
...
CREATE OR REPLACE FUNCTION public.confirmar_pago_bono(p_bono_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  /* ═══ EL ROLLOVER, POR TIPO DE BONO Y FAIL-CLOSED ═══════════════════════
     Llegó la plata ⇒ ahora sí se extiende la vigencia de los paquetes viejos.
     **Pero la regla NO es la misma para los dos tipos, y no es un filtro más:**
       · guardería → por `(familia, prestador)`
       · paseo     → por `(familia, prestador, SERVICIO)` — el paquete de paseo
         es de una oferta concreta, no del lugar entero.
     *Escribir esto como un `if` de guardería con el paseo colgado al lado haría
     que el próximo tipo se cuele por el `else` y nadie lo note.*

     🔴 UN TIPO SIN RAMA DECLARADA REVIENTA. `bonos_tipo_valido` admite hoy dos;
     el día que admita un tercero, esta función **falla ruidosamente** en vez de
     confirmarlo sin rollover. *Un tercer tipo que pasa en silencio es saldo que
     la familia pagó y no recibe, y no deja síntoma: el bono queda pagado.* */
  IF v_b.tipo_servicio = 'guarderia_dia' THEN
    UPDATE bonos b
  ...
  ELSIF v_b.tipo_servicio = 'paseo' THEN
    /* 🔴 ESTE ROLLOVER SE MUDÓ DESDE `comprar_paquete_salidas`, donde vivía en
       el ACTO DE COMPRAR. Con el bono naciendo `pendiente` ahí regalaba vigencia
       por plata no cobrada — el mismo defecto que ya se curó en guardería, y la
       razón por la que este paso va ANTES de cambiarle el estado inicial: *si el
       estado cambia primero, el rollover se pierde en silencio.* */
    UPDATE bonos b
       SET fecha_vencimiento = v_b.fecha_vencimiento,
           pago_metadata = COALESCE(b.pago_metadata,'{}'::jsonb)
                           || jsonb_build_object('rollover_extendido_por', p_bono_id,
                                                 'rollover_en', now())
     WHERE b.familia_id = v_b.familia_id AND b.prestador_id = v_b.prestador_id
       AND b.prestador_servicio_id = v_b.prestador_servicio_id
       AND b.tipo_servicio = 'paseo' AND b.estado = 'activo'
       AND b.estado_pago = 'pagado'
       AND b.unidades_usadas < b.unidades_total AND b.id <> p_bono_id;

  ELSE
    RAISE EXCEPTION 'tipo_de_bono_sin_rollover_declarado: %', v_b.tipo_servicio
      USING ERRCODE='22023';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.confirmar_pago_bono(uuid) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  /* ═══ EL ROLLOVER SE MUDÓ A LA CONFIRMACIÓN ══════════════════════════════
     ...
     *Un beneficio otorgado por un cobro que no ocurrió es la misma clase de
     ...
     `confirmar_pago_bono`, que corre cuando la plata entró. */
...
-- ═══ CINTURÓN — el rollover del PASEO se ejerce y se mide EN LA FILA ═══════
DO $c$
DECLARE v_fam uuid; v_prest uuid; v_serv uuid; v_user uuid;
        v_viejo uuid; v_nuevo uuid; v_venc_antes date; v_venc_despues date;
        v_r jsonb; v_n int; v_g uuid;
BEGIN
  SELECT b.familia_id, b.prestador_id, b.prestador_servicio_id, b.user_id
    INTO v_fam, v_prest, v_serv, v_user
    FROM bonos b WHERE b.tipo_servicio='paseo' LIMIT 1;
  IF v_fam IS NULL THEN RAISE EXCEPTION 'cinturon: sin bono de paseo con que DISCRIMINAR'; END IF;

  -- un paquete de paseo VIEJO, pagado, con saldo y vencimiento cercano
  ...
  VALUES (v_prest, v_user, v_fam, 'paseo', v_serv, 4, 1, 40, 10,
          public.hoy_local(), (public.hoy_local() + 3)::date, 'activo','pagado','EC')
  RETURNING id INTO v_viejo;
  SELECT fecha_vencimiento INTO v_venc_antes FROM bonos WHERE id=v_viejo;

  -- y el NUEVO, pendiente, con vencimiento lejano
  INSERT INTO bonos (prestador_id, user_id, familia_id, tipo_servicio, prestador_servicio_id,
    unidades_total, unidades_usadas, precio_total, precio_por_unidad,
    fecha_compra, fecha_vencimiento, estado, estado_pago, country_code, pago_expira_en)
  VALUES (v_prest, v_user, v_fam, 'paseo', v_serv, 5, 0, 50, 10,
          public.hoy_local(), (public.hoy_local() + interval '1 month')::date,
          'activo','pendiente','EC', now() + interval '15 minutes')
  RETURNING id INTO v_nuevo;

  v_r := public.confirmar_pago_bono(v_nuevo);
  IF COALESCE((v_r->>'ok')::boolean,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'cinturon: no confirmo el bono de PASEO: %', v_r::text;
  END IF;

  /* 🔴 SE MIDE EN LA FILA, no en el retorno. *`bonos_extendidos: 1` es lo que
     la función DICE; la fecha de la fila es lo que PASÓ.* */
  SELECT fecha_vencimiento INTO v_venc_despues FROM bonos WHERE id=v_viejo;
  IF v_venc_despues <= v_venc_antes THEN
    RAISE EXCEPTION 'cinturon: el rollover del PASEO no extendio nada (% -> %)',
                    v_venc_antes, v_venc_despues;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM bonos WHERE id=v_viejo
                  AND pago_metadata->>'rollover_extendido_por' = v_nuevo::text) THEN
    RAISE EXCEPTION 'cinturon: extendio sin dejar la marca de quien lo extendio';
  END IF;

  /* 🔴 CONTROL NEGATIVO: el rollover del paseo NO alcanza a un bono de GUARDERÍA
     del mismo hogar. *Sin este brazo, «resuelve por tipo» y «extiende todo lo
     del hogar» darían el mismo verde.* */
  SELECT id INTO v_g FROM bonos WHERE familia_id=v_fam AND tipo_servicio='guarderia_dia'
     AND estado='activo' AND estado_pago='pagado' LIMIT 1;
  IF v_g IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM bonos WHERE id=v_g
                AND pago_metadata->>'rollover_extendido_por' = v_nuevo::text) THEN
      RAISE EXCEPTION 'cinturon: el rollover de PASEO toco un bono de GUARDERIA';
    END IF;
  ELSE
    RAISE NOTICE 'cinturon M14: control negativo NO EJERCIDO — sin bono de guarderia pagado en ese hogar';
  END IF;

  -- (c) un tipo sin rama declarada REVIENTA
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='confirmar_pago_bono'
     AND pg_get_functiondef(p.oid) LIKE '%tipo_de_bono_sin_rollover_declarado%';
  IF v_n <> 1 THEN RAISE EXCEPTION 'cinturon: falta el fail-closed por tipo'; END IF;

  -- (d) el nombre viejo YA NO EXISTE (el renombre es real, no un alias)
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='confirmar_pago_paquete_guarderia';
  IF v_n <> 0 THEN RAISE EXCEPTION 'cinturon: el nombre viejo sigue vivo'; END IF;

  RAISE NOTICE 'cinturon M14: 5/5 OK (paseo confirma · rollover EXTENDIO medido en la fila · con su marca · NO toco guarderia · fail-closed por tipo · nombre viejo muerto)';
  RAISE EXCEPTION 'FIXTURE_ROLLBACK_OK';
EXCEPTION WHEN OTHERS THEN
  IF SQLERRM = 'FIXTURE_ROLLBACK_OK' THEN
    RAISE NOTICE 'cinturon M14: fixture deshecho por subtransaccion, residuo 0';
  ELSE RAISE;
  END IF;
END $c$;
