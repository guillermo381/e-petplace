-- S97-A · LA VOZ DEL HANDSHAKE (D-815)
--
-- El push LLEGÓ al teléfono (verificado por camino real: `entregadas:1` y la
-- notificación en la bandeja) y decía «Tienes una novedad en e-PetPlace ·
-- Abre la app para verla». Ese es el fallback del transporte cuando
-- `_voz_notificacion` devuelve `{}` — y devolvía `{}` porque **el tipo nació
-- sin su rama.** Hueco propio de S97-A, cerrado el mismo día.
--
-- 🔴 EL GENÉRICO NO SIRVE PARA ESTE AVISO, y no es prolijidad:
--    «tienes una novedad» es una invitación a mirar CUANDO PUEDAS. Acá del
--    otro lado hay un profesional PARADO en el mostrador esperando.
--    *Un aviso que no transmite que alguien espera se lee después — y
--    «después» es cuando el vet ya resolvió el problema de otra forma, que
--    es justo el camino que este handshake existe para evitar.*
--
-- LA VOZ DICE TRES COSAS: quién pide · para qué · que la espera es AHORA.
-- Sin nombre de negocio dice «un negocio» — jamás inventa uno (L-139).
--
-- 76(g): NO RIGE — DDL sobre el cuerpo de una función. Cero filas tocadas.
--
-- ⚠️ EMPALME, NO REESCRITURA: el cuerpo se tomó de `pg_get_functiondef` VIVO
-- y se le insertó UNA rama antes del `ELSE`, verificando que el ancla
-- apareciera exactamente una vez y que las ramas de PRIMER NIVEL subieran
-- en 1 (el primer intento contó `WHEN` a secas y abortó: la rama tiene
-- `CASE WHEN` adentro — el guard frenó por la razón equivocada y aun así
-- evitó escribir a ciegas). *Reescribir a mano una función de 30 ramas es
