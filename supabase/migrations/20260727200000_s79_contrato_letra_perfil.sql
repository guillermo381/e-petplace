-- ═════════════════════════════════════════════════════════════════════
-- S79-A · T4.6 — EL CONTRATO DE LETRA_PERFIL_S79 v1.1, APLICADO.
--
-- LIBERADO por el gate en dispositivo del founder (27-Jul-2026: el alta
-- completa contra Paseos Shyris — sala de espera, Places, radio escrito
-- por el founder, activación, la carta con su firma). Las DOS
-- condiciones del freno se cumplieron: la FIRMA (Tanda 4) + el GATE.
--
-- El cuerpo de abajo es EL CONTRATO VERBATIM
-- (docs/relevamientos/2026-07-27-s79a-CONTRATO-letra-perfil.sql, v1.1
-- con las enmiendas T3.4 y la condición estado='activo' de la firma) —
-- generado por copia del archivo, no re-tipeado. Pre-check corrido
-- ANTES de aplicar (lo manda el preámbulo del contrato):
--   · crear_prestador_inicial vivo CONSERVA el COALESCE(…,5) ✓
--   · obtener_paseadores_disponibles vive con 3 parámetros ✓
--   · las columnas nuevas NO existen ✓
--
-- 76(g), DECLARADA: NO RIGE — DDL aditivo (columnas nullable sin
-- DEFAULT), cero backfill, cero anclas sobre datos vivos.
-- REVERSA (escrita en Tanda 2, enmendada en T3):
--   docs/relevamientos/2026-07-27-s79a-REVERSA-letra-perfil.sql
-- ═════════════════════════════════════════════════════════════════════
begin;
