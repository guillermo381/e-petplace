-- ═══════════════════════════════════════════════════════════════════════════
-- S107 · EL AVISO DEL REVERSO SE BIFURCA POR DESTINATARIO
--
-- Firma del founder: 27-ago-2026.
-- Reversa escrita ANTES:
--   docs/relevamientos/S107-CERT-REVERSA-20260827220000-aviso-por-destinatario.sql
-- 76(g): **NO RIGE** — DDL pura, cero filas tocadas.
--
-- 🔴 EL DEFECTO, medido sobre el PRIMER `pago_reversado` real de la historia
--    del producto (cita `d41c9dea`, telemedicina, 27-ago 15:46):
--
--      «Una cita quedo cancelada — El banco devolvio el pago, asi que la cita
--       se cancelo y el horario volvio a quedar libre.
--       **No se te cobrara nada por esta cita.**»
--
--    Eso le llego al PRESTADOR. Y tiene dos defectos, no uno:
--      ① le dice la causa financiera del cliente;
--      ② la ultima frase esta escrita para quien PAGA — al prestador le dice
--         que pierde un cobro.
--
-- 🔴 LA CONTRADICCION QUE LO ENMARCA, y vale mas que el caso:
--    `apps/prestador/.../cita-no-disponible.tsx:30` declara que decirle al
--    prestador que se revirtio el pago **expondria un movimiento financiero del
--    cliente**. *La pantalla del prestador protegia esa privacidad y su
--    notificacion la publicaba. Dos superficies del mismo producto con
--    criterios opuestos sobre el mismo dato, y ninguna sabia de la otra.*
--
-- ⇒ La bifurcacion pasa de SUJETO a (SUJETO x DESTINATARIO). **No es agregar un
--   aviso: el que existia estaba mal dirigido.**
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ── CINTURON: contra la definicion VIVA, con sus tres brazos ────────────────
DO $cinturon$
DECLARE v_p text; v_f text; v_v text;
BEGIN
  v_p := (_voz_notificacion('pago_reversado', NULL, NULL,
           jsonb_build_object('sujeto','cita','para','prestador')))->>'mensaje';
  v_f := (_voz_notificacion('pago_reversado', NULL, NULL,
           jsonb_build_object('sujeto','cita','para','familia')))->>'mensaje';
  v_v := (_voz_notificacion('pago_reversado', NULL, NULL,
           jsonb_build_object('sujeto','pedido')))->>'mensaje';

  -- ① el prestador NO se entera de plata
  IF v_p IS NULL OR v_p LIKE '%pago%' OR v_p LIKE '%banco%' OR v_p LIKE '%cobr%' THEN
    RAISE EXCEPTION 'CINTURON: la voz del PRESTADOR habla de plata -> %', left(coalesce(v_p,'(null)'),90);
  END IF;

  -- ② la familia SI, y con el literal de C
  IF v_f IS NULL OR v_f NOT LIKE '%el banco devolvio el pago%' OR v_f NOT LIKE '%No te cobramos nada%' THEN
    RAISE EXCEPTION 'CINTURON: la voz de la FAMILIA no trae el literal de C -> %', left(coalesce(v_f,'(null)'),90);
  END IF;

  -- ③ el vendedor queda INTACTO (no se rompio lo que funcionaba)
  IF v_v IS NULL OR v_v NOT LIKE '%devuelve los productos al stock%' THEN
    RAISE EXCEPTION 'CINTURON: se rompio la voz del VENDEDOR -> %', left(coalesce(v_v,'(null)'),90);
  END IF;

  -- ④ fail-closed: sin `para`, cita cae en el texto que menos dice
  IF (_voz_notificacion('pago_reversado', NULL, NULL,
        jsonb_build_object('sujeto','cita')))->>'mensaje' IS DISTINCT FROM v_p THEN
    RAISE EXCEPTION 'CINTURON: sin `para` la cita NO cae en el texto del prestador';
  END IF;

  -- ⑤ y el movedor manda los DOS avisos con claves distintas
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
       WHERE n.nspname='public' AND p.proname='mover_sujeto_por_reverso'
         AND pg_get_functiondef(p.oid) LIKE '%:familia%'
         AND pg_get_functiondef(p.oid) LIKE '%:prestador%') <> 1 THEN
    RAISE EXCEPTION 'CINTURON: el movedor no distingue las dos claves de dedup';
  END IF;

  RAISE NOTICE 'CINTURON VERDE: prestador sin plata · familia con el literal de C · vendedor intacto · fail-closed · dos claves';
END $cinturon$;
