/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ⚠️ **NÚMERO PUESTO POR A AL APLICAR (20260907800000).** D la entregó SIN número
   a propósito, y la razón está abajo. **El texto es el de D, sin editar** —
   sólo esta nota y la ruta de la reversa cambiaron.

   **A verificó antes de aplicar:** corrió el arnés de D concatenado con ESTE
   archivo en una transacción con ROLLBACK, **porque el ledger ya no estaba en
   594 como cuando D lo midió** (D dejó esa condición escrita: *una fecha no es
   un estado*). Verde, y residuo 0 medido después — la función no quedó viva.

   🔴 **SIN NÚMERO A PROPÓSITO (texto de D).** No tomo números de migración: dos veces en un
   ...
   **Reversa:** `docs/relevamientos/S112-A-REVERSA-20260907800000-barrido-adopcion.sql`, **escrita ANTES**.
