-- ═══════════════════════════════════════════════════════════════════════════
-- S89-A · §1a — LAS CINCO VOCES DEL MOTOR A TUTEO (FIRMA founder, 6-ago-2026)
--
-- La firma resolvió la divergencia de acentos que D midió (relevamiento
-- D-539 §los-dos-acentos): tuteo neutro ÚNICO. Migran las CINCO voces del
-- lote S88 que hablaban en voseo: plan_renovado · plan_renovacion_proxima ·
-- paquete_vence · programa_vence · procedimiento_agendado. El LADO A LADO
-- viejo→nuevo queda depositado como letra en
-- docs/relevamientos/2026-08-06-s89a-LETRA-lado-a-lado-tuteo-motor.md.
-- Solo cambia el acento: ni datos, ni estructura, ni destinatarios.
--
-- 76(g): NO RIGE — un CREATE OR REPLACE, cero backfill.
-- D-662: cero contrato tocado (la voz viaja en datos de intenciones nuevas).
-- L-140: no nace función; proacl intacto.
-- REVERSA: docs/relevamientos/2026-08-06-s89a-REVERSA-tuteo-motor.sql.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ── CINTURÓN: cero voseo residual en las voces del motor ────────────────────
DO $cint$
DECLARE v_src text;
BEGIN
  SELECT p.prosrc INTO v_src FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
  WHERE n.nspname='public' AND p.proname='_voz_notificacion';
  IF v_src ~ 'Podés|podés|querés|Coordiná' THEN
    RAISE EXCEPTION 'cinturon_tuteo: queda voseo en _voz_notificacion';
  END IF;
END $cint$;
