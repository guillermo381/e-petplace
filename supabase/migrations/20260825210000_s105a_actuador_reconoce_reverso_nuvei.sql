-- ══════════════════════════════════════════════════════════════════════════
-- S105-A · EL ACTUADOR RECONOCE EL REVERSO DE NUVEI  (tanda 2a de `D-923`)
--
-- EL DEFECTO, medido y no supuesto: el reverso REAL `DF-2102135` (25-ago-2026)
-- llegó al buzón, se autenticó bien, y **el actuador lo descartó como
-- `desconocido`** — porque un reverso trae `status=2`, exactamente igual que
-- un rechazo, y `_pago_aprobado` no distingue. *El circuito del reverso tenía
-- todo construido salvo la primera pregunta.* `L-318` otra vez: motor sin
-- puerta — sólo que acá la puerta estaba, y miraba el campo equivocado.
--
-- QUÉ HACE ESTA MIGRACIÓN — y sobre todo QUÉ **NO** HACE:
--   ✅ reconoce el reverso por `status_detail` y se lo entrega a
--      `registrar_reverso_nuvei` (pista D), que ya es idempotente,
--      fail-closed por proveedor y valida la ventana.
--   ❌ **NO mueve el sujeto.** La compra sigue diciendo `pagada` sobre plata
--      devuelta. Eso es `D-923` y NO se hace acá por una razón que es de mesa
--      y no de motor: son CUATRO sujetos (compra · pedido · cita ·
--      recurrencia/suscripción) y el estado destino de tres de ellos es
--      decisión de producto. Censo servido a la mesa el 25-ago.
--   ❌ NO toca el ledger. Medido: la compra reversada **no tiene evento
--      económico** (el devengo nace al entregar/cerrar, no al cobrar), y
--      `aplicar_reembolso` **no tiene un solo llamador en toda la base**.
--
-- 76(g) — VEDA DE ESCRITURA: **NO RIGE.** DDL puro sobre dos funciones, sin
-- backfill, sin anclas, sin tocar una fila de negocio. El cinturón sí escribe,
-- y por eso corre en SUBTRANSACCIÓN QUE SE DESHACE SOLA (`L-406`).
--
-- REVERSA escrita ANTES: `docs/relevamientos/S105-A-REVERSA-20260825210000.sql`
-- ══════════════════════════════════════════════════════════════════════════

/* ① EL VOCABULARIO DE «ESTO ES UN REVERSO», EN UN SOLO LUGAR.
   🔴 QUÉ ESTÁ MEDIDO Y QUÉ ES HEREDADO, porque no es lo mismo:
     · `7`  — **MEDIDO** sobre `DF-2102135`: reverso total, `ReversedByMerchant`.
     · `34` — **HEREDADO de la doc de Nuvei** (reverso PARCIAL). Nunca lo
              pedimos —la edge no manda `order.amount`— pero puede llegar, y
              `registrar_reverso_nuvei` ya lo trata aparte como hallazgo.
   *No se agregan códigos que no se puedan citar contra algo.* El día que
   Nuvei devuelva otro `status_detail` de reverso, va a morir en `desconocido`
   con su número escrito en el detalle — que es dónde hay que ir a buscarlo. */
CREATE OR REPLACE FUNCTION public._nuvei_status_detail_es_reverso(p_detail text)
RETURNS boolean
LANGUAGE sql IMMUTABLE
SET search_path TO 'public', 'pg_temp'
AS $$
  /* 🔴 `COALESCE` NO ES PROLIJIDAD — lo pidió el cinturón en su primer intento.
     Sin él, `NULL IN ('7','34')` devuelve **NULL, no `false`**, y esto pasaba
     a ser un booleano de TRES valores adentro de un guard de plata. Hoy el
     `IF` del actuador trataría ese NULL como "no entrar" —que da la respuesta
     correcta por accidente—, pero *un guard que acierta por cómo trata SQL a
     los nulos es un guard que deja de acertar cuando alguien lo niega, lo
     compone con un OR, o lo mueve a un `CASE`.* Un evento sin `status_detail`
     NO es un reverso, y ahora lo dice en vez de callarse. */
  SELECT COALESCE(p_detail IN ('7','34'), false)
$$;

REVOKE ALL ON FUNCTION public._nuvei_status_detail_es_reverso(text) FROM PUBLIC, anon;

/* ② EL ACTUADOR — cuerpo byte-idéntico al vigente salvo el bloque (3bis).
   El camino de aprobación no se movió un signo: se verifica abajo. */
...
BEGIN
  ...
  THEN
    ...
    RETURN jsonb_build_object('ok', true,
      ...
      'sujeto_movido', false, 'deuda', 'D-923');
  END IF;
...
-- ══════════════════════════════════════════════════════════════════════════
-- CINTURÓN — con ROJO PRODUCIDO contra el EVENTO REAL, no contra un fixture.
-- 🔴 Escribe, así que corre en subtransacción que se deshace sola (`L-406`):
--    el intento vuelve exactamente a como estaba antes de decidir el assert.
-- ══════════════════════════════════════════════════════════════════════════
DO $cint$
DECLARE
  v_ev      uuid;
  ...
  v_vocab   boolean;
BEGIN
  -- ⓪ el vocabulario discrimina (y no dice que sí a cualquier cosa)
  SELECT _nuvei_status_detail_es_reverso('7')
     AND _nuvei_status_detail_es_reverso('34')
     AND NOT _nuvei_status_detail_es_reverso('3')
     AND NOT _nuvei_status_detail_es_reverso(NULL)   -- ← el que cazó el tri-estado
    INTO v_vocab;
  IF NOT COALESCE(v_vocab, false) THEN
    RAISE EXCEPTION 'CINTURÓN: el vocabulario no discrimina';
  END IF;

  SELECT w.id INTO v_ev FROM webhook_events w
   WHERE w.payload->'transaction'->>'status_detail' = '7'
     AND w.proveedor = 'nuvei'
   ORDER BY w.recibido_en DESC LIMIT 1;

  IF v_ev IS NULL THEN
    /* Sin el evento real no hay nada que probar, y **no se fabrica uno**:
       un fixture acá probaría mi propio regex, no el payload del proveedor. */
    RAISE EXCEPTION 'CINTURÓN: no hay evento de reverso real contra el cual medir';
  END IF;

  SELECT i.id, i.estado INTO v_int, v_antes FROM pagos_intentos i
   WHERE i.proveedor_transaction_id = (
     SELECT w.payload->'transaction'->>'id' FROM webhook_events w WHERE w.id = v_ev)
     AND i.proveedor = 'nuvei';
  IF v_int IS NULL THEN
    RAISE EXCEPTION 'CINTURÓN: el evento real no tiene intento — el reconocedor no se puede probar';
  END IF;

  -- ① EL ROJO, reproducido: se lleva el intento al estado que tenía cuando el
  --    evento llegó (`aprobado`) para que el camino sea el de verdad.
  UPDATE pagos_intentos SET estado='aprobado' WHERE id = v_int;

  v_res := aplicar_evento_de_pago(v_ev);

  v_llego  := (v_res->>'motivo') = 'reverso_nuvei';
  SELECT estado INTO v_estado FROM pagos_intentos WHERE id = v_int;

  -- ② SE DESHACE ANTES DE DECIDIR — si el assert falla, el dato ya volvió.
  UPDATE pagos_intentos SET estado = v_antes WHERE id = v_int;
  UPDATE webhook_events SET resultado='desconocido' WHERE id = v_ev;

  IF NOT v_llego THEN
    RAISE EXCEPTION 'CINTURÓN: el reverso NO fue reconocido — motivo=% (antes moría en status_no_aprobado)',
      COALESCE(v_res->>'motivo','?');
  END IF;

  /* 🔴 EL VEREDICTO DE D DEPENDE DEL RELOJ, y eso está bien: su ventana es
     mismo-día y antes de las 17:00 Guayaquil. Lo que ESTA migración prueba es
     que el evento **deja de morir en `status_no_aprobado` y LLEGA a D** — el
     código que D devuelva (`reversado` · `ya_reversado` ·
     `fuera_de_ventana_*`) es su decisión, no la del reconocedor.
     *Exigir un código concreto sería atar este cinturón a la hora del día.* */
  RAISE NOTICE 'CINTURÓN VERDE · reconocido=% · codigo_de_D=% · sujeto_movido=% · estado_intento_restaurado=%',
    v_llego, COALESCE(v_res#>>'{acto,codigo}','?'), v_res->>'sujeto_movido', v_antes;
END $cint$;
