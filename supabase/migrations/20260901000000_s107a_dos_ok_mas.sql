/* ═══════════════════════════════════════════════════════════════════════════
   S107-A · LOS DOS `ok` QUE FALTABAN — firma del founder (31-ago-2026)
   ═══════════════════════════════════════════════════════════════════════════
   La cura del sujeto desconocido dejó **OCHO** salidas más con
   `ok: true, aplicado: false`. Se censaron una por una y **se voltearon DOS**,
   por la misma razón que la primera:

   · **`reverso_sin_intento`** — *plata devuelta que no encontró su intento.*
     Un `ok` ahí dice **«resuelto»** sobre dinero que quedó sin dueño.
   · **`monto_no_coincide`** — *plata que llegó por otro monto*, el caso donde
     más importa que alguien mire.

   > **Los dos son estados que exigen intervención humana, y un `ok` los hace
   > invisibles.**

   ✅ **LAS SEIS QUE QUEDAN SE CONSERVAN, POR DECISIÓN Y NO POR OLVIDO** —
   `actuador_apagado` · `evento_no_autenticado_o_no_server` ·
   `sin_consulta_verificada` · `sin_dev_reference` · `status_no_aprobado` ·
   `cita_ya_pagada`. **Son el camino normal**: un pago rechazado que no se
   aplica, una cita ya pagada, un motor apagado a propósito. *Voltear un `ok`
   sin mirar qué significa cada uno sería el mismo apuro que esto vino a
   corregir.*

   ✏️ **Y una corrección de número mía:** al reportarlo dije *«quedan siete»* y
   **eran ocho**. *Un conteo aproximado sobre una lista que se puede contar es
   una medición que no se hizo.*

   ⚖️ VEDA 76(g): **NO RIGE**. ↩️ REVERSA escrita ANTES:
      `docs/relevamientos/S107-A-REVERSA-20260901000000-dos-ok-mas.sql`
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══ CINTURÓN — SE CUENTA SOBRE EL OBJETO, NO CON UN REGEX ═════════════════
   ⚠️ La v1 de este cinturón usó un patrón con `[^;]*` y **dio 0 legítimas
   cuando había 6**: el rojo era del INSTRUMENTO, no del código. *Misma clase
   que `L-437` — el 0 era de la forma que el patrón miraba.* Contar substrings
   no puede equivocarse así.

   🔑 **El brazo que impide voltear de más:** si quedaran MENOS de 6, se tocó
   alguna del camino normal. Sin él, un reemplazo que volteara las ocho también
   daría verde. */
DO $cinturon$
DECLARE
  v_src text; v_total int;
  q  text := chr(39);
  v_pat text; v_g1 text; v_g2 text;
BEGIN
  v_pat := q||'ok'||q||', true, '||q||'aplicado'||q||', false';
  v_g1  := q||'ok'||q||', false, '||q||'aplicado'||q||', false';

  SELECT pg_get_functiondef(p.oid) INTO v_src FROM pg_proc p
    JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='aplicar_evento_de_pago';

  v_total := (length(v_src) - length(replace(v_src, v_pat, ''))) / length(v_pat);

  RAISE NOTICE E'\n═══ CINTURON · los dos ok ═══\n  salidas que aun dicen ok:true -> %  (esperado 6: las legitimas)\n  reverso_sin_intento sigue en ok:true -> %\n  monto_no_coincide  sigue en ok:true -> %',
    v_total,
    position(v_pat||', '||chr(10)||'        '||q||'motivo'||q||', '||q||'reverso_sin_intento' IN v_src) > 0,
    strpos(v_src, v_pat||', '||q||'motivo'||q||', '||q||'monto_no_coincide') > 0;

  IF strpos(v_src, v_g1) = 0 THEN
    RAISE EXCEPTION 'CINTURON ROJO: ninguna salida quedo en ok:false — los dos volteos no entraron';
  END IF;
  IF v_total <> 6 THEN
    RAISE EXCEPTION 'CINTURON ROJO: quedan % salidas en ok:true y tienen que ser 6 (las legitimas)', v_total;
  END IF;
END $cinturon$;
