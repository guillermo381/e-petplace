-- ═══════════════════════════════════════════════════════════════════════════
-- S89-A · ADJUDICACIÓN DE MESA — `evento_id` viaja en `cita_solicitada`
--
-- Del fork de C (mapa de destinos del prestador): un cita_solicitada sin
-- evento_id cae al fallback de mascota o a no-tocable. La intención pasa a
-- portar el evento de timeline de la cita (eventos_mascota — el dominio de
-- la columna evento_id de notificacion_intencion); el cita_id sigue en
-- datos. El destino-por-tipo al catálogo queda REGISTRADO para el tren de
-- push (adjudicación: diseño allá, no acá).
--
-- 76(g): NO RIGE · D-662: cero contrato · L-140: proacl intactos.
-- REVERSA: docs/relevamientos/2026-08-06-s89a-REVERSA-evento-id-solicitada.sql
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  FOREACH v_fn IN ARRAY ARRAY['confirmar_cita_pagada','reservar_salida_paquete'] LOOP
    SELECT p.prosrc INTO v_src FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
    WHERE n.nspname='public' AND p.proname=v_fn;
    IF v_src NOT LIKE '%p_evento_id%' THEN
      RAISE EXCEPTION 'cinturon_evento_id: % no porta el referente', v_fn;
    END IF;
  END LOOP;
END $cint$;
